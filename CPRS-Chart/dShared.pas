...
type
  ...
  public
    ...
    procedure AddTemplateNode(Tree: TTreeView; var EmptyCount: integer;
                              ...
                              HideNamespacePartOfName : boolean=false); //kt added HideNamespacePartOfName variable
    function ExpandNode(Tree: TTreeView; Node: TTreeNode;
                        var EmptyCount: integer; AllowInactive: boolean = FALSE;
                        HideNamespacePartOfName : boolean=false): boolean; //kt added HideNamespacePartOfName variable
    ...
    property  TagIndex: longint read FTagIndex write FTagIndex; //kt added
  end;
...
uses
  uCarePlan, //kt
  StrUtils, //kt
  fDrawers, rTemplates, uCore, uTemplateFields, uEventHooks, VA508AccessibilityRouter;
...
const
  ...
  DialogImageXRef: array[0..DialogConvMax, Boolean] of integer = (
                  //Boolean: F,  T
                            (5,  18),  //0   5->White multi-doc in closed folder  18->Lt Gray Dlg in closed folder
                            (6,  19),  //1   6->White multi-doc in open folder    19->Lt Gray Dlg in open folder
                            (8,  20),  //2   8->Gray multi-doc in closed folder   20->Drk Gray Dlg in closed folder
                            (9,  21),  //3   9->Gray multi-doc in open folder     21->Drk Gray Dlg in open folder
                            (11, 22),  //4  11->White multi-doc2 in closed folder 22->Lt Gray Dlg2 in closed folder
                            (12, 23),  //5  12->White multi-doc2 in open folder   23->Lt Gray Dlg2 in open folder
                            (14, 24),  //6  14->Gray multi-doc2 in closed folder  24->Drk Gray Dlg2 in closed folder
                            (15, 25)); //7  15->Gray multi-doc in open folder     25->Drk Gray Dlg2 in open folder
...
begin
  ...
  if(assigned(Node.Data)) then begin
    with TTemplate(Node.Data) do begin
      if (RealType = ttDoc) and (IsReminderDialog) then begin
        Result := RemDlgIdx[(PersonalOwner <= 0)]
      end else if (RealType = ttDoc) and (IsCOMObject) then begin
        Result := COMObjIdx[COMObjectOK(COMObject)]
      end else if EmbeddedDialog then begin  //kt added block  6/16
        ...
        Result := 30;  //This hard codes all imbedded dialogs to 1 dialog icon, regardless of child status etc...
      end else begin
        Typ := TemplateType;
        if(Exclude and (Typ in [ttDocEx, ttGroupEx])) then begin
          if(not assigned(Node.Parent)) or (TTemplate(Node.Parent.Data).RealType <> ttGroup) then begin
            case Typ of
              ttDocEx: Typ := ttDoc;
        ...
        if(Dialog and (Typ in [ttGroup, ttGroupEx])) then begin
          for i := 0 to DialogConvMax do begin
            //kt doc  DialogImageXRef[i, <Boolean for If Dialog>]
            if(Result = DialogImageXRef[i, FALSE]) then begin
...
var
  ...
  var TmplName : string; //kt
  begin
    TmplName := tmpl.PrintName;  //kt
    if HideNamespacePartOfName then TmplName := uCarePlan.StripNamespacePartOfName(TmplName, VEFATemplateMode);  //kt
    NewNode := Tree.Items.AddChildObject(Owner, TmplName, tmpl);  //kt changed tmpl.PrintName --> TemplName
...
begin
  ...
  begin
    Done := FALSE;
    NewNode := nil;
    if (VEFATemplateMode= cptemCarePlan) and
      (tmpl.RealType in [ttDoc, ttDocEx]) and                                        //kt added
      not (TemplateIsCarePlan(tmpl.PrintName)) then begin                            //kt added
      done := true;                                                                  //kt added breakpoint line
      exit;                                                                          //kt added
    end;                                                                             //kt added
...
function TdmodShared.ExpandNode(Tree: TTreeView; Node: TTreeNode;
                                var EmptyCount: integer; AllowInactive: boolean = FALSE;
                                HideNamespacePartOfName : boolean=false): boolean;
                                //kt added HideNamespacePartOfName variable
var
  TmpNode: TTreeNode;
  tmpl: TTemplate;
  i :integer;
...
begin
  ...
  begin
    ...
    for i := 0 to tmpl.Items.Count-1 do
      AddTemplateNode(Tree, EmptyCount, TTemplate(tmpl.Items[i]), AllowInactive, Node, HideNamespacePartOfName);  //kt added HideNamespacePartOfName
...
begin
  Err := nil;
  try
    if Pos('{DYNAMIC}',tmpl.PrintName)>0 then tmpl.ClearChildren;

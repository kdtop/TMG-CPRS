...
uses
  ...
  VA508AccessibilityManager, VA508ImageListLabeler, StrUtils;
...
type
  ...
  TfrmTemplateEditor = class(TfrmBase508Form)
    ...
    lblNotes: TLabel;
    reNotes: TRichEdit;
    reBoilChangeTimer: TTimer;
    procedure HandleREBoilChangeTimerEvent(Sender: TObject);  //kt added  6/16
    procedure mnuInsertFormulaClick(Sender: TObject);  //kt //TMG-CPRS
...
//    FOldPersonalTemplate: TTemplate;
  ...
  protected
    ...
    procedure CheckEnableOKandLinkedButtons(Node : TTreeNode);  //kt added
    ...
    procedure InsertFormulaMenuItem(MenuFile : TMenuItem);                //kt  //VEFA-261
    procedure UpdateNumLinkedDx(Count : integer);                         //kt
    procedure SetMode(Mode : TcpteMode);                                  //kt
    function PropText : string;                                           //kt
    procedure btnAddProblemClick(Sender: TObject);                        //kt
    procedure tvSharedCustomDrawItem(Sender: TCustomTreeView; Node: TTreeNode; //kt
                           State: TCustomDrawState; var DefaultDraw: Boolean); //kt
    procedure tvSharedChanging(Sender: TObject; Node: TTreeNode; var AllowChange: Boolean); //kt
    procedure InsertBPFormulaMenuItem(MenuFile : TPopupMenu);             //kt  //VEFA-261
    function FindFunction(SelText:string):string;                         //kt  //VEFA-261
    procedure HandleREBoilChange;                                         //kt added 6/16
    procedure SetREBoilBackgroundColor;                                   //kt 6/16
  end;

//kt procedure EditTemplates(Form: TForm; NewTemplate: boolean = FALSE; CopiedText: string = ''; Shared: boolean = FALSE);
procedure EditTemplates(Form: TForm;
                        NewTemplate: boolean = FALSE;
                        CopiedText: string = '';
                        Shared: boolean = FALSE;
                        ExpandStr : string = '';         //kt
                        SelectStr : string = '';         //kt
                        Mode : TcpteMode= cptemNormal;   //kt
                        ProbData : TCPProblemData = nil  //kt
                        );
...
uses dShared, uCore, rTemplates, fTemplateObjects, uSpell, fTemplateView,
  ...
  fCarePlanLinkedDxs, fCarePlanPicker,fFormulaHelper, //VEFA-261 added this line
  rMisc, fFindingTemplates;

const
  //kt replaced with function --> PropText = ' Template Properties ';
//  GroupTag = 5;
  BPDisplayOnlyFld = 0;
  BPFirstLineFld = 1;
  BPOneItemOnlyFld = 2;
...
type
  //kt original --> TTypeIndex = (tiTemplate, tiFolder, tiGroup, tiDialog, tiRemDlg, tiCOMObj);
  TTypeIndex = (tiTemplate, tiFolder, tiGroup, tiDialog, tiRemDlg, tiCOMObj, tiEmbeddedDialog);  //kt 6/16
...
//  TypeTag: array[TTypeIndex] of integer = (7, 6, 8, -8, 7, 7);
  ...
  //kt original --> TypeTag: array[TTypeIndex] of TTemplateType = (ttDoc, ttClass, ttGroup, ttDialog, ttDoc, ttDoc);
  TypeTag: array[TTypeIndex] of TTemplateType = (ttDoc, ttClass, ttGroup, ttDialog, ttDoc, ttDoc, ttDialog);  //kt 6/16
  //kt removed ForcedIdx: array[boolean, TTypeIndex] of integer = ((0, 1, 2, 3, 4, 5), (-1, 0, 1, 2, -1, -1));
  //kt removed IdxForced: array[boolean, 0..5] of TTypeIndex = ((tiTemplate, tiFolder, tiGroup, tiDialog, tiRemDlg, tiCOMObj),
  //kt           (tiFolder, tiGroup, tiDialog, tiNone, tiNone, tiNone));
...
//kt original --> procedure EditTemplates(Form: TForm; NewTemplate: boolean = FALSE; CopiedText: string = ''; Shared: boolean = FALSE);
procedure EditTemplates(Form: TForm;
                        NewTemplate: boolean = FALSE;
                        CopiedText: string = '';
                        Shared: boolean = FALSE;
...
begin
  ...
  try
    ...
    begin
      ...
      if (NewTemplate) then begin
        btnNewClick(frmTemplateEditor);
        if (CopiedText <> '') then
        begin
          if mode <> cptemNormal then begin   //kt added block
            edtName.Text := CheckNamespaceName(NewTemplateName, FVEFAMode);  //kt added
            VEFANewTemplate := true;
          end;
          end else begin  //kt original block below
...
begin
  FVEFAMode := cptemNormal; //kt -- Default object mode variable to to normal
  ...
  //kt --- begin mod ---
  VEFANewTemplate := false;
  tvShared.OnCustomDrawItem   := tvSharedCustomDrawItem;
  tvPersonal.OnCustomDrawItem := tvSharedCustomDrawItem;
  tvShared.OnChanging := tvSharedChanging;
  tvPersonal.OnChanging := tvSharedChanging;
  btnAddProblem := TBitBtn.Create(Self);
  with btnAddProblem do begin
    Parent := pnlMenuBar;
    Height := 22;  Width := 150;
    Top    := 0;   Left  := btnNew.Left - Width - 5;
...
{ Don't mess with the order of the following commands! }
  ...
  BtnApply.Enabled := BackupDiffers;
  //SetFormPosition(Self);
  InsertFormulaMenuItem(mnuEdit);  //VEFA-261 added
  InsertBPFormulaMenuItem(popBoilerplate);  //VEFA-261 added
end;

procedure TfrmTemplateEditor.InsertBPFormulaMenuItem(MenuFile : TPopupMenu);
//VEFA-261 added entire function
var InsertPoint, i : integer;
begin
  InsertPoint := MenuFile.Items.Count-7;
  for i := 0 to MenuFile.Items.Count-1 do begin
    if MenuFile.Items[i] <> mnuBPInsertField then continue;
    InsertPoint := i+1;
    break;
  end;
  mnuBPInsertFormula := TMenuItem.Create(MenuFile);
  with mnuBPInsertFormula do begin
    Caption := 'Insert/Edit Fo&rmula';
    OnClick := mnuInsertFormulaClick;
  end;
  mnuBPInsertFormula.ShortCut := Shortcut(Word('R'),[ssCtrl]);
  if (InsertPoint >=0) and (InsertPoint < MenuFile.Items.Count) then begin
    MenuFile.Items.Insert(InsertPoint,mnuBPInsertFormula);
  end else begin
    MenuFile.Items.Add(mnuBPInsertFormula);
  end;
  mnuBPInsertFormula.Enabled := false;
end;

procedure TfrmTemplateEditor.InsertFormulaMenuItem(MenuFile : TMenuItem);
//VEFA-261 added entire function
var InsertPoint, i : integer;
begin
  InsertPoint := MenuFile.Count-8;
  for i := 0 to MenuFile.Count-1 do begin
    if MenuFile.Items[i] <> mnuInsertField then continue;
    InsertPoint := i+1;
    break;
end;
  mnuInsertFormula := TMenuItem.Create(MenuFile);
  with mnuInsertFormula do begin
    Caption := 'Insert/Edit Fo&rmula';
    OnClick := mnuInsertFormulaClick;
  end;
  mnuInsertFormula.ShortCut := Shortcut(Word('R'),[ssCtrl]);
  if (InsertPoint >=0) and (InsertPoint < MenuFile.Count) then begin
    MenuFile.Insert(InsertPoint,mnuInsertFormula);
  end else begin
    MenuFile.Add(mnuInsertFormula);
  end;
  mnuInsertFormula.Enabled := false;
end;
...
procedure TfrmTemplateEditor.tvSharedChanging(Sender: TObject; Node: TTreeNode; var AllowChange: Boolean);
//kt added function
begin
  inherited;
  AllowChange := true; //kt -- may restore later
  exit;
  AllowChange := false;
...
procedure TfrmTemplateEditor.tvSharedCustomDrawItem(Sender: TCustomTreeView; Node: TTreeNode;
                                                    State: TCustomDrawState; var DefaultDraw: Boolean);
//kt added function
begin
  inherited;
  if FVEFAMode = cptemNormal then exit;
...
var
  ...
  Template: TTemplate;
  //kt EnableOK : boolean; //kt

begin
  reBoil.Color := clWindow; //kt 6/16
  ChangeTree(TTreeView(Sender));
  Something := assigned(Node);
  if Something then begin
    Template := TTemplate(Node.Data);
    //kt EnableOK  := (FVEFAMode = cptemNormal) or uCarePlan.NodeMatchesMode(Node,False,FVEFAMode);  //kt
    Something := assigned(Template);
    if Something then begin
      if (Sender = tvPersonal) then begin
        ok := FCanEditPersonal;
        if ok and (Template.PersonalOwner = 0) and IsTemplateLocked(Node) then begin
          ok := FALSE;
        end;
      end else begin
        ok := FCanEditShared;
      ...
      ShowInfo(Node);
      if Template.Changed then SetREBoilBackgroundColor; //kt 6/16
    end;
  end;
  ...
  CheckEnableOKandLinkedButtons(Node);  //kt added
end;

procedure TfrmTemplateEditor.CheckEnableOKandLinkedButtons(Node : TTreeNode);
//kt added entire function
var
  EnableOK : boolean; //kt
begin
  if assigned(Node) then begin
    EnableOK  := (FVEFAMode = cptemNormal) or uCarePlan.NodeMatchesMode(Node,False,FVEFAMode);  //kt
  end else begin
    EnableOK := False;
  end;
  btnOK.Enabled := EnableOK;         //kt
  if assigned(btnAddProblem) then btnAddProblem.Enabled := EnableOK; //kt
end;
...
var
  ...
  lts: string;
  i : integer;  //kt
//kt also made some formatting changes...
begin
  ...
  try
    if (assigned(Node)) then begin
      ...
      with FShowingTemplate do begin
        ClearName := FALSE;
        ClearRB := FALSE;
        ClearAll := FALSE;
        UpdateNumLinkedDx(LinkedDxsCount);  //kt added
        ...
        if (RealType in AllTemplateRootTypes) then begin
          ...
          case RealType of
            ttDoc:   begin
                       if IsReminderDialog then begin
                         Idx := tiRemDlg
                       end else if IsCOMObject then begin
                         Idx := tiCOMObj
                       end else begin
                         Idx := tiTemplate;
                       end;
            ...
            ttGroup: begin
                       if (EmbeddedDialog) then begin    //kt added 6/16
                         Idx := tiEmbeddedDialog;        //kt added 6/16
                       end else if (Dialog) then begin   //kt added "end else" to beginning of line
                         Idx := tiDialog;
                       end else begin
                         Idx := tiGroup;
                       end;
          ...
          if not FForceContainer then begin
            //kt original --> cbxType.Items.Add('Template');
            cbxType.Items.AddObject('Template', Pointer(tiTemplate));
          end;
          //kt original --> cbxType.Items.Add('Folder');
          cbxType.Items.AddObject('Folder', pointer(tiFolder));
          //kt original --> cbxType.Items.Add('Group Template');
          cbxType.Items.AddObject('Group Template', pointer(tiTemplate));
          //kt original --> cbxType.Items.Add('Dialog');
          cbxType.Items.AddObject('Dialog', pointer(tiDialog));
          if (not FForceContainer) then begin
            if (FCanDoReminders or CanDoCOM) then begin
              //kt original --> cbxType.Items.Add('Reminder Dialog');
              cbxType.Items.AddObject('Reminder Dialog', pointer(tiRemDlg));
            end;
            if (CanDoCOM) then begin
              //kt original --> cbxType.Items.Add('COM Object');
              cbxType.Items.AddObject('COM Object', pointer(tiCOMObj));
            end;
          end;
          cbxType.Items.AddObject('Embedded Dialog', pointer(tiEmbeddedDialog));  //kt added 6/16
          //kt original cbxType.ItemIndex := ForcedIdx[FForceContainer, Idx];
          //kt mod to replace ForcedIdx[FForceContainer, Idx];
          cbxType.ItemIndex := 0;
          for i := 0 to cbxType.Items.Count - 1 do begin
            if TTypeIndex(cbxType.Items.Objects[i]) <> Idx then continue;
            cbxType.ItemIndex := i;
            break;
          end;
          //kt end mod
          ...
          if (RealType in [ttGroup, ttClass]) and (Children <> tcNone) and
            (dmodShared.InDialog(Node)) then begin
...
begin
  ...
  CheckEnableOKandLinkedButtons(FCurTree.Selected);  //kt added
end;
...
begin
  ...
  ClearBackup;
  UnlockAllTemplates;
  if FVEFAMode = cptemNormal then dmodShared.Reload;   //kt added conditional
  btnAddProblem.Free;                                  //kt added
  VEFATemplateMode := cptemNormal;                     //kt  Restore to Normal mode.  Used by GetItems of nodes in dShared etc.
  dmodShared.Reload;
end;
...
begin
  if (Assigned(FCurTree)) then begin
    ...
    if (Curok) then begin
      ...
      AllowDown := AllowUp;
      if (Node.Index < (Node.Parent.Count - 1)) then begin
        AllowDown := TRUE
      end else begin
        if (not AllowSet) then begin
      ...
      if (FTreeControl[Tree, tcCopy].Enabled) then begin
        if (Tree = ttShared) then begin
      ...
      for i := low(TTemplateTreeControl) to high(TTemplateTreeControl) do begin
    ...
    if (FCurTree = tvShared) and (FCanEditShared) then begin
      ...
      if (FCurTree = tvPersonal) and (FCanEditPersonal) then begin
        btnNew.Enabled := TRUE
      end else begin
...
procedure TfrmTemplateEditor.SetREBoilBackgroundColor;
//kt added 6/16
...
begin
  ...
  if not Template.BoilerplateContainsScript then exit;
  if User.CanMakeTemplatesWithScripts then exit;
  reBoil.Color := clYellow;
end;


procedure TfrmTemplateEditor.HandleREBoilChange;
//kt added procedure 6/16
//NOTE: copied from reBoilChange(Sender: TObject);
var
...
begin
  if ((not FUpdating) and (assigned(FCurTree)) and (assigned(FCurTree.Selected))) then begin
    ...
    if (Template.Boilerplate <> TmpBPlate) then begin
      if CanClone(FCurTree.Selected) then begin
        DoRefresh := Clone(FCurTree.Selected);
        if (DoRefresh) then begin
          Template := TTemplate(FCurTree.Selected.Data);
        end;
        if assigned(Template) and Template.CanModify then begin
          DoInfo := FALSE;
          if (Template.Boilerplate = '') or (TmpBPlate = '') then begin
            DoInfo := TRUE;
          end;
          Template.Boilerplate := TmpBPlate;
          TTemplate(FCurTree.Selected.Data).Gap := StrToIntDef(edtGap.Text, 0);
          if (Template.RealType = ttGroup) then begin
          ...
          SetREBoilBackgroundColor; //kt 6/16
        end;
      end;
      btnApply.Enabled := TRUE;
...
procedure TfrmTemplateEditor.HandleREBoilChangeTimerEvent(Sender: TObject);
//kt added
begin
  inherited;
  HandleREBoilChange();
  //do other things here if wanted...
end;
...
begin
  //NOTE: this will fire HandleREBoilChangeTimerEvent() after 400 ms
  reBoilChangeTimer.Interval := 400;
  reBoilChangeTimer.Enabled := true;
  //NOTE: Below moved to HandleREBoilChange, to allow timer delay.  Thus everything
  //      here doesn't have to be done immediately with every user keystroke.
  {
  if ((not FUpdating) and (assigned(FCurTree)) and (assigned(FCurTree.Selected))) then begin
    Template := TTemplate(FCurTree.Selected.Data);
    TmpBPlate := reBoil.Lines.Text;
...
begin
  ...
  begin
    ...
    AdjustControls4FontChange;
    MoveCopyButtons;
    if VEFANewTemplate then edtNameOldChange(nil); //kt
    VEFANewTemplate := false;                      //kt
  end;
  //kt CheckEnableOKandLinkedButtons(nil);  //kt added
end;
...
begin
  ...
  if FVEFAMode = cptemCarePlan then frmTemplateObjects.InsertingIntoCarePlan := true;  //kt
end;
...
begin
  edtName.Text := CheckNamespaceName(edtName.Text, FVEFAMode);  //kt added
  Warn := (ActiveControl <> btnCancel) and (BadTemplateName(edtName.Text, FVEFAMode));  //kt added FVEFAMode
  CheckEnableOKandLinkedButtons(FCurTree.Selected);  //kt added
...
begin
  ...
  begin
    ...
    mnuInsertFormula.Enabled := ok;  //VEFA-261
...
var
  ...
  tmpl: TTemplate;
  txt: string;
  DBControlData : TDBControlData; //kt addded 5/16
...
begin
  ...
  begin
    ...
    if (not tmpl.DialogAborted) then begin
      ...
      DBControlData.MsgNoSave;  //kt 5/16
    end;
    if (Move) then
...
procedure TfrmTemplateEditor.mnuInsertFormulaClick(Sender: TObject);
//VEFA-261 added
var
  FormulaHelperForm: TfrmFormulaHelper;
  //InitialText : string;

begin
  inherited;
  FormulaHelperForm := TfrmFormulaHelper.Create(Self, reBoil);
  FormulaHelperForm.InitialFormula := FindFunction(reBoil.SelText);
  //test BP text to see if contains full formula, if so Formula helper initial
  if FormulaHelperForm.ShowModal = mrOK then begin
    //NOTE: If in careplane mode, then result should be wrapped with '-=[  ]=-'
    reBoil.SelText := FormulaHelperForm.GetFormula;
  end;
  FreeAndNil(FormulaHelperForm);
end;

function TfrmTemplateEditor.FindFunction(SelText:string):string;
//VEFA-261 added
var
   TagPos: integer;
begin
   ...
   if TagPos > 0 then begin
     ...
     if TagPos > 0 then begin
        ...
        messagedlg('Selected Text does not contain a complete formula',mtWarning,[mbOK],0);
    end;
  end;
 end;
...
var
  ImgIdx: integer;
  AType: TTypeIndex;  //kt added

begin
  cbxType.Canvas.FillRect(Rect);
  AType := TTypeIndex(cbxType.Items.Objects[Index]); //kt added 6/16
  //kt original --> case IdxForced[FForceContainer, Index] of
  case AType of
    tiTemplate: ImgIdx := 4;
    tiFolder:   ImgIdx := 3;
    tiGroup:    ImgIdx := 5;
    ...
    tiRemDlg:   ImgIdx := 27;
    tiCOMObj:   ImgIdx := 28;
    //kt NOTICE!!  I added an image in mgTemplates TImageList
    //   Icon is named EmbeddedDialogIcon.bmp, and stored in:
    //   P:\vista\vista-art\icons\CPRS Icons
    //   I added it as a new image, which happened to be "30", as below.
    //   Must be added to new CPRS when porting up!
    tiEmbeddedDialog: ImgIdx := 30; //kt added 6/16
  else
    ImgIdx := ord(tiNone);
  end;
  if ImgIdx >= 0 then
...
begin
  if ((not FUpdating) and (assigned(FCurTree)) and (assigned(FCurTree.Selected))) then begin
    ...
    if tg >= 0 then begin
      {
      //kt ------- begin mod ----------
      if (FCareplanModeActive) and (tg > ord(tiDialog)) then begin
        ShowMsg('Invalid type for a CarePlan Template'+#13#10+
                'Changing to default ''Template'' type.');
        cbxType.ItemIndex := ord(tiTemplate);
        cbxType.Text := cbxType.Items.Strings[cbxType.ItemIndex];
        exit;
      end;
      //kt ------- end mod ------------
      }
      if CanClone(FCurTree.Selected) then begin
        //kt original --> idx := IdxForced[FForceContainer, tg];
        idx := TTypeIndex(cbxType.Items.Objects[cbxType.ItemIndex]); //kt added 6/16
        if (idx = tiRemDlg) and (not (GetLinkType(FCurTree.Selected) in [ltNone, ltTitle])) then begin
          ...
          if assigned(Template) and Template.CanModify then begin
            ttyp := TypeTag[idx];
            if (not FForceContainer) or (not (idx in [tiTemplate, tiRemDlg])) then begin
              Template.EmbeddedDialog := (idx = tiEmbeddedDialog);  //kt added 6/16
              ...
              if (Template.RealType = ttDoc) and (idx = tiRemDlg) then begin
                Template.IsReminderDialog := TRUE
              end else begin
                Template.IsReminderDialog := FALSE;
              end;
              if (Template.RealType = ttDoc) and (idx = tiCOMObj) then begin
            ...
            for i := 0 to Template.Nodes.Count - 1 do begin
              Node := TTreeNode(Template.Nodes.Objects[i]);
              Node.ImageIndex := dmodShared.ImgIdx(Node);
              Node.SelectedIndex := dmodShared.ImgIdx(Node);
            end;
...
begin
  if (assigned(Node)) and assigned(Node.Data) then begin
    if (TTreeView(Node.TreeView) = tvPersonal) then begin
      Template := TTemplate(Node.Data);
      if Template.IsCOMObject or (Template.FileLink <> '') then begin
        Result := FALSE
      end else begin
        Result := TTemplate(GetFirstPersonalNode(Node).Data).CanModify
      end;
...
procedure TfrmTemplateEditor.SetMode(Mode : TcpteMode);
//kt Added function
var Name : string;
begin
  FVEFAMode := Mode;
  VEFATemplateMode := Mode; //used by GetItems of nodes in dShared etc.
  ...
  if FVEFAMode = cptemCarePlan then begin
    btnAddProblem.Visible := true;
    btnAddProblem.Enabled := false;
    btnNew.Width := 150;  //shrink button to give more room.
  end else begin
    btnAddProblem.Visible := false;
    btnNew.Width := 182;  //original size
  end;
  //Self.Caption := Name + ' Template Editor';
  gbProperties.Caption := Name + ' properties';
  if FVEFAMode = cptemNormal then Name := '' else Name := Name + ' ';   //  1/24/12
  Self.Caption := Name + 'Template Editor';
  ...
  mnuNewTemplate.Caption := '&New ' + Name + ' Template';
  ...
  mnuEditTemplateFields.Caption := 'Edit ' + Name + ' Template Fields';
  mnuImportTemplate.Caption := '&Import ' + Name + ' Template';
  mnuExportTemplate.Caption := '&Export ' + Name + ' Template';
  mnuRefresh.Caption := '&Refresh ' + Name + ' Templates';
  mnuTemplateIconLegend.Caption := Name + ' Template Icon Legend';
  mnuInsertField.Caption := 'Insert ' + Name + ' Template &Field';
  mnuGroupBoilerplate.Caption := 'Group ' + Name + ' Template & Boilerplate';
end;

function TfrmTemplateEditor.PropText : string;
//kt added function
begin
  Result := ' ' + TEMPLATE_MODE_NAME[FVEFAMode] + ' Properties ';
end;

procedure TfrmTemplateEditor.UpdateNumLinkedDx(Count : Integer);
//kt added function
begin
  btnAddProblem.Caption := 'Linked &Dx''s (' + IntToStr(Count) + ')';
end;

procedure TfrmTemplateEditor.btnAddProblemClick(Sender: TObject);
//kt added function
var

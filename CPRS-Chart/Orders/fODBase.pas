...
var
  ...
  begin
    ...
    if IEN <> 0 then begin
      ...
      if assigned(frmODBase) and (frmODBase.FOrderAction = ORDER_EDIT) then begin
        //kt orginal --> CheckBoilerplate4Fields(tmp, cptn)
        CheckBoilerplate4Fields(tmp, cptn, [], DBControlData);  //kt
      end else begin
        ExecuteTemplateOrBoilerPlate(tmp, IEN, LType, nil, cptn, DocInfo);
      end;
    end else begin
      //kt 5/16 original --> CheckBoilerplate4Fields(tmp, cptn);
      CheckBoilerplate4Fields(tmp, cptn, [], DBControlData);
    end;
...
begin
  ...
  begin
    ...
    //kt 9/11 note: A better solution to the line above would be to trigger a selection
    //kt 9/11      of a valid patient at this point, if I could figure out how to do that...
    x := OrderChecksOnDisplay(FillerID);  //kt 9/11 <-- Expects Patient.DFN to hold valid number...
    StatusText('');

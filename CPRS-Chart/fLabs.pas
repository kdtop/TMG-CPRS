...
type
  ...
  tLabsInfoRec = record   //kt added this record 2/2017
    Data : TStringList;  //owned by creator of record variable    //format:  .Strings[i]='LabName^LabValue^Flags^Units^ReferenceRange'
    Notes : TStringList; //owned by creator of record variable    //Format: Notes comment is stored as simple text lines.
    LabSrc : string;  //e.g. 'CH' or 'MI'
    LabFMDT : TFMDateTime; //dates of returned labs
    Date : String;  //external format of LabFMDT
    Specimen : string;
    uFormat : string;
    FlagLevel : integer;  //Worst flag found in lab list.  0-> undefined, 1->normal 2->abnormal, 3->Critical
  end;
  ...
  TfrmLabs = class(TfrmHSplit)
    ...
    PopupMenu2: TPopupMenu;       //kt documentation: this was not originally being used.  Added as popupmenu for grdLab 2/17
    Print1: TMenuItem;
    Copy1: TMenuItem;
    ...
    Action1: TMenuItem;
    ManLabs: TMenuItem;
    CreateNewNote: TMenuItem;
    mnuCreateLabNote: TMenuItem;
    popTMGGridPopup: TPopupMenu;                             //kt
    popGrdCreateLabNote: TMenuItem;                          //kt 2/17
    mnuSendLabAlert: TMenuItem;                              //kt 2/17
    mnuSendLabAlert2: TMenuItem;
    ...
    procedure grdLabDrawCell(Sender: TObject; ACol, ARow: Integer; Rect: TRect; State: TGridDrawState);
    procedure FormShow(Sender: TObject);  //TMG    //kt
    procedure ManLabsClick(Sender: TObject);
  ...
  private
    ...
    FLabData : TStringList;    //kt added 2/17
    FNotes : TStringList;      //kt added 2/17
    LastCheckedLabs : string;
    procedure AlignList;
  ...
  public
    ...
    function GetCurrentLabNotesHTMLTable : string;              //kt added 2/17
    function GetPickedLabNotesHTMLTable : string;               //kt added 2/17
  end;
...
var text: string;
    RectForText: TRect;
    FlagText : string;
const cHIGHLIGHT_COLOR = $66FFFF;  //#ffff66
begin
  inherited;
  FlagText := grdLab.Cells[3, ARow];
  if (FlagText <> 'N') and (FlagText <> '') and (ACol > 0) and (ARow > 0) then begin
    text := grdLab.Cells[ACol, ARow];
    // Fill rectangle with color
    grdLab.Canvas.Brush.Color := cHIGHLIGHT_COLOR;
    grdLab.Canvas.FillRect(Rect);
    // Next, draw the text in the rectangle
    //grdLab.Canvas.Font.Color := clWhite;
    RectForText := Rect;
...
begin
  ...
  LabRowObjects.Free;
  FLabData.Free;  //kt added 2/17
  FNotes.Free;    //kt added 2/17
end;
...
begin
  ...
  if LastCheckedLabs<>Patient.DFN+'^'+Piece(aitems[0], '^', 3) then begin
     agrid.Color := GetLabGridColor(Patient.DFN,Piece(aitems[0], '^', 3),inttostr(User.DUZ),FirstSeen);
     lblFirstSeen.caption := FirstSeen;
     LastCheckedLabs := Patient.DFN+'^'+Piece(aitems[0], '^', 3);
  end;
  with agrid do
  begin
  ...
  try
    ...
    begin
      ...
      begin
        ...
        TMGHeight := GridVisibleHeight(grdLab) + 80;  //kt added 12/15
...
procedure TfrmLabs.FormShow(Sender: TObject);
//kt added function
begin
  inherited;
  //kt if uTMGOptions.ReadString('SpecialLocation','')<>'FPG' then begin
  ManLabs.enabled := AtFPGLoc();
end;
...
begin
  ...
  if StrToIntDef(OrderIFN,0) > 0 then begin
    ...
    tvReportsClick(self); //kt added 1/10/17 because the report would not actually be selected or display
  end else begin
    if Patient.Inpatient then lstDates.ItemIndex := 2 else lstDates.ItemIndex := 4;
    tvReports.Selected := tvReports.Items.GetFirstNode;
    //kt --- begin mod to seek for the date of the labs if I wanted to earlier dates -----
    tvReportsClick(self);
    DT := piece(AlertData,'|',2);
    DT := piece(DT,';',2);
    FMDT := StrToFMDateTime(DT);
    FMDT := FMDT-0.0001; //NOTE: this may cause problems with rollover of 0 -> 99
    GetInterimGrid(FMDT,-1);
    //kt End mod--------------
  end;
...
procedure TfrmLabs.GetVisibleLabs(var LabInfo: tLabsInfoRec);
//kt added 2/17
//This should return the RPC data for the currently displayed lab data.
//NOTE: I am only writing this for "CH" labs for now.  I don't have any "MI" data
//      to test with...
  function FlagLevel(Flag : string) : integer;
  //Note: The equivalent mapping on server side is done in LRWRITE^TMGLRW01
  //1->normal 2->abnormal, 3->Critical
  begin
    Result := 1;
    Flag := UpperCase(Flag);
    if (Flag='H') or (Flag='L') then begin
      Result := 2;
    end else if (Flag='HH') or (Flag='LL') or (Flag='C') or (Flag = '*') then begin
      Result := 3;
    end;
  end;
...
var
  ADate1 : TFMDateTime;
  Data, Notes, TempSL : TStringList;
  RowCount, i, FLevel : integer;
  Flag, DataStr, s : string;
begin
  LabInfo.LabSrc := '';
  LabInfo.Date := '';
  LabInfo.LabFMDT := 0;
  LabInfo.Date := '';
  LabInfo.FlagLevel := 1; //default to normal.
  Data := LabInfo.Data;  //owned by creator of LabInfo
  Notes := LabInfo.Notes;  //owned by creator of LabInfo
  if assigned(Data) then Data.Clear;
  if assigned(Notes) then Notes.Clear;
  TempSL := TStringList.Create;
  try
    if uMostRecent.Count > 0 then begin
      TempSL.Assign(uMostRecent);  //most recent 'CH' results(?)
    end else begin
      ADate1 := StrToFloat(lblDateFloat.Caption);
      ADate1 := ADate1 + 0.00000001;  //crazy inverse dates on server requires this!
      TempSL.Assign(InterimGrid(Patient.DFN, ADate1, 1, 1));  //NOTE: could I just use uMostRecent rather than calling RPC again?
    end;
    If TempSL.Count < 1 then exit;
    s := TempSL.Strings[0];
    LabInfo.LabSrc := Piece(s, '^', 2);
    if LabInfo.LabSrc <> 'CH' then exit;  //not supporting MI at this time, could be added later....
    LabInfo.LabFMDT := MakeFMDateTime(Piece(s, '^', 3));
    LabInfo.Date := FormatFMDateTime('mm/dd/yy hh:nn:ss',LabInfo.LabFMDT);
    LabInfo.uFormat := Piece(s, '^', 9);
    RowCount := StrToIntDef(Piece(s, '^',1),0);
    for i := 1 to TempSL.Count - 1 do begin
      s := TempSL.Strings[i];
      if i <= RowCount then begin
        Flag := Trim(Piece(s,'^',4));
        FLevel := FlagLevel(Flag);
        If FLevel > LabInfo.FlagLevel then LabInfo.FlagLevel := FLevel;
        DataStr := Trim(Piece(s,'^',2)) + '^' + //lab name, e.g. 'WBC'
                   Trim(Piece(s,'^',3)) + '^' +  //lab value, e.e. '12'
                   Trim(Piece(s,'^',4)) + '^' +  //lab flags, if any
                   Trim(Piece(s,'^',5)) + '^' +  //lab units
                   Trim(Piece(s,'^',6));         //lab reference range
        Data.Add(DataStr);
      end else begin
        Notes.Add(s);
      end;
            ...
            If aID = '1:MOST RECENT' then
                  ...
                  begin               // TMG added this else    11/5/18
                   ...
                   { end
                    ...
                    (aID = '9:MICROBIOLOGY') or
                    (aID = '20:ANATOMIC PATHOLOGY') or
                    (aID = '2:BLOOD BANK') or
                    (aID = '10:LAB STATUS') or
                    (aID = '3:ALL TESTS BY DATE') or
                    (aID = '21:CUMULATIVE') or
                    (aID = '27:AUTOPSY') then  // else if (aID = '9:MICROBIOLOGY') or (aID = '20:ANATOMIC PATHOLOGY') or (aID = '2:BLOOD BANK') or (aID = '10:LAB STATUS') or (aID = '3:ALL TESTS BY DATE') or (aID = '21:CUMULATIVE') or (aID = '27:AUTOPSY') then
                  begin
...
procedure TfrmLabs.mnuCopyResultsTableClick(Sender: TObject);
...
begin
  inherited;
  LabInfo.Data := TStringList.Create;
  LabInfo.Notes := TStringList.Create;
  GetVisibleLabs(LabInfo);
  Level := IntToStr(LabInfo.FlagLevel);  //1->normal 2->abnormal, 3->Critical
  Info := Patient.DFN + '^' + FloatToStr(LabInfo.LabFMDT) + '^CH';
...
function TfrmLabs.GetCurrentLabsHTMLTable : string;
//kt added 2/17
...
function TfrmLabs.GetAbnormalCurrentLabsHTMLTable : string;
//kt added 2/17
...
function TfrmLabs.GetCurrentLabNotesHTMLTable : string;
//kt added 2/17
var
  LabInfo: tLabsInfoRec;
begin
...
function TfrmLabs.GetPickedLabNotesHTMLTable : string;
//kt added 2/17
var
  LabInfo: tLabsInfoRec;

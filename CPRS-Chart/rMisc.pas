...
//kt 5/26/14 --> procedure SetUserBounds(var AControl: TControl);
procedure SetUserBounds2(AName: string; var v1, v2, v3, v4: integer);
procedure ForceUserBoundsInsideWorkArea(var Left, Top, Width, Height : integer); //kt 5/26/14
...
{
//kt removed entire function 5/26/14
RATIONALE: This is called from only one place: TfrmFrame.LoadSizesForUser, and that was changed.
I had made changes to the function (seen below), before realizing that the entire procedure should be removed
...
begin
  ...
  //kt original --> if (x = '0,0,0,0') and (AControl is TForm) then begin
  //kt original -->   TForm(AControl).WindowState := wsMaximized
  //kt -- begin mod --
  if (AControl is TForm) then begin
    if (x = '0,0,0,0')  then begin
      TForm(AControl).WindowState := wsMaximized
    end else begin
      //Below will get info about form position from SizeHolder instead of uBounds
      SetFormPosition(TForm(AControl));  //kt <-- this is core of change
    end;
  //kt -- end mod --
  end else begin
...
procedure ForceUserBoundsInsideWorkArea(var Left, Top, Width, Height : integer);
//kt added function 5/26/14
var Rect: TRect;
begin
  Rect.Left := Left;
  Rect.Top := Top;

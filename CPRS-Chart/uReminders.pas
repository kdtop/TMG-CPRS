...
type
  ...
  public
    DBControlData : pointer;  //kt 5/16 added  NOTE: 1) Not owned here.  2 ) Will actually be TDBControlData type
    ...
    //procedure ClearMHTest(RIEN: String);  AGP Not needed leaving this code in just in case
  ...
  //kt NOTE: TRemDlgElement's are owned by TReminderDialog.FElements
...
var
  RemForm: TRemForm; //kt doc  This is a globally-scoped RECORD giving info about current reminder form
...
{ OtherReminder string format:
  ...
  RemindersInProcess: TORStringList = nil; //kt doc Holds and owns list of TReminders.  Freed in ClearReminderData() 
...
begin
  ...
  if (Copy(SIEN,1,1) = RemCode) then begin
...
begin
  TempSL := GetDlgSL;
  if Piece(TempSL[0],U,2)='1' then begin
    Self.RemWipe := 1;
  end;
  idx := -1;
  repeat
    ...
    if(idx >= 0) then begin
      if(not assigned(FElements)) then begin
      ...
      with Element do begin
        DBControlData := Self.DBControlData; //kt added 5/16
        FReminder := Self;
        FRec1 := TempSL[idx];
        FID := Piece(FRec1, U, 2);
...
begin
  Result := nil;
  if(assigned(FElements)) then begin
    Y := 0;
    for i := 0 to FElements.Count-1 do begin
      Elem := TRemDlgElement(FElements.Objects[i]);
      if (not assigned(Elem.FParent)) then begin
  ...
  if(AParent.ControlCount = 0) then begin
    with TVA508StaticText.Create(AOwner) do begin
      Parent := AParent;
...
var
  ...
  begin
    ...
    Entry := GetDialogEntry(BaseParent, EID + IntToStr(Integer(BaseParent)), AText, TDBControlData(DBControlData));
        ...
        if(OK) then begin
          ...
          case pt of
            ...
            ptLevelSeverity, ptSkinResults, ptSkinReading: begin
              ...
              if pt = ptVisitLocation then begin
                DefLoc := GetDefLocations;
                if DefLoc.Count > 0 then begin
                  idx := 1;
                  for j := 0 to DefLoc.Count-1 do begin
                    LocText := piece(DefLoc[j],U,2);
                    if LocText <> '' then begin
                      if (LocText <> '0') and (IntToStr(StrToIntDef(LocText,0)) = LocText) then begin
                        LocFound := FALSE;
                        for k := 0 to Ctrl.cbo.Items.Count-1 do begin
                          if(piece(Ctrl.cbo.Items[k],U,1) = LocText) then begin
                            LocText := Ctrl.cbo.Items[k];
                            LocFound := TRUE;
                            break;
                          end;
                        end;
                        if not LocFound then begin
                          LocText := '';
                        end;
                      end else begin
                        LocText := '0^'+LocText;
              ...
              if(Ctrl.cbo.ItemIndex < 0) then begin
            ...
            end; //case ptVisitLocation, ptLevelUnderstanding, ptSeries, ptReaction, ptExamResults, ptLevelSeverity, ptSkinResults, ptSkinReading:

            ptWHPapResult:  begin
              if FData<>nil then begin
                if (TRemData(FData[i]).DisplayWHResults)=true then begin
                  NextLine(Y);
...
//                      TExposedComponent(Ctrl.WHChk.Flbl).Caption);
                  ...
                  if (LineWidth + Ctrl.WHChk.Check2.Width) > PWidth - 10 then begin
                    LineWidth := WrapLeft;
                  ...
                  if (LineWidth + Ctrl.WHChk.Check3.Width) > PWidth - 10 then begin
                    LineWidth := WrapLeft;
...
//                  TExposedComponent(Ctrl.WHChk.Flbl).Caption)+4;
              ...
              Ctrl.WHChk.Height := TextHeightByFont(Ctrl.WHChk.Font.Handle, Ctrl.WHChk.Caption)+4;
              if (LineWidth + Ctrl.WHChk.Width) > PWidth - 10 then begin
                LineWidth := WrapLeft;
                Y := Ctrl.WHChk.Flbl.Top + Ctrl.WHChk.Flbl.Height;
                Nextline(Y);
              end;
              ...
              if piece(Prompt.FRec4,u,12)='1' then begin
                ...
                if (LineWidth + Ctrl.WHChk.FPrintNow.Width) > PWidth - 10 then begin
                  LineWidth := WrapLeft;
                ...
                MinX :=PWidth;
                if (Ctrl.WHChk.FButton.Top + Ctrl.WHChk.FButton.Height) > (Ctrl.WHChk.FPrintNow.Top + Ctrl.WHChk.FPrintNow.Height) then begin
            ...
            end; //case ptVisitDate:

            ptPrimaryDiag, ptAdd2PL, ptContraindicated: begin
              ...
              MinX := Ctrl.cb.Width;
            end; //case ptPrimaryDiag, ptAdd2PL, ptContraindicated:

            else begin  //else of CASE
              if(pt = ptSubComment) then begin
                Ctrl.cb := TCPRSDialogCheckBox.Create(AOwner);
                Ctrl.ctrl.Parent := AParent;
                Ctrl.cb.Checked := (Prompt.Value = '1');
                Ctrl.cb.Caption := Prompt.Caption;
                ...
                UpdateColorsFor508Compliance(Ctrl.cb);
                MinX := Ctrl.cb.Width;
              end else if pt = ptVitalEntry then begin
                vt := Prompt.VitalType;
                if(vt = vtPain) then begin
                  Ctrl.cbo := TCPRSDialogComboBox.Create(AOwner);
                  Ctrl.ctrl.Parent := AParent;
                  Ctrl.cbo.Style := orcsDropDown;
                  Ctrl.cbo.Pieces := '1,2';
                  ...
                  if(ElementChecked = Self) then begin
                  ...
                  if(vt in [vtTemp, vtHeight, vtWeight]) then begin
                    HasVCombo := TRUE;
                ...
                DoLbl := TRUE;
              end else if pt = ptDataList then begin
                Ctrl.cbo := TCPRSDialogComboBox.Create(AOwner);
                Ctrl.ctrl.Parent := AParent;
                Ctrl.cbo.Style := orcsDropDown;
                Ctrl.cbo.Pieces := '12';
                ...
                if Self.Historical then begin
                  EncDt := DateTimeToFMDateTime(Date)
                end else begin
                  EncDt := RemForm.PCEObj.VisitDateTime;
                end;
                if assigned(Prompt.FData.FChoicesActiveDates) then begin {csv active/inactive dates}
                  for m := 0 to (Prompt.FData.FChoices.Count - 1) do begin
                    for n := 0 to (TStringList(Prompt.FData.FChoicesActiveDates[m]).Count - 1) do begin
                      ...
                      if (EncDt >= ActDt) and (EncDt <= InActDt) then begin
              ...
              end else if(pt = ptMHTest) or ((pt = ptGaf) and (MHDLLFound = true)) then begin
                Ctrl.btn := TCPRSDialogButton.Create(AOwner);
                Ctrl.ctrl.Parent := AParent;
                Ctrl.btn.OnClick := Prompt.DoMHTest;
                Ctrl.btn.Caption := Prompt.ForcedCaption;
                if Piece(Prompt.FData.FRec3,U,13)='1' then begin
                ...
                DoLbl := TRUE;
              end else if ((pt = ptGAF)) and (MHDLLFound = false) then begin
                Ctrl.edt := TCPRSDialogFieldEdit.Create(AOwner);
                Ctrl.ctrl.Parent := AParent;
                Ctrl.edt.Text := Prompt.Value;
                ud := TUpDown.Create(AOwner);
                ...
                Ctrl.edt.OnKeyPress := Prompt.EditKeyPress;
                Ctrl.edt.OnChange := Prompt.PromptChange;
                if(User.WebAccess and (GAFURL <> '')) then begin
          ...
          //if(DoLbl) then
          if(DoLbl) and ((pt <> ptWHNotPurp) and (pt <> ptWHPapResult)) then begin
            Req := Prompt.Required;
            if (not Req) and (pt = ptGaf) and (MHDLLFound = false) then begin
              Req := (Piece(Prompt.FData.FRec3,U,13) = '1');
            end;
            ProcessLabel(Req, Prompt.FParent.Enabled, AParent, Ctrl.Ctrl);
            if assigned(lblCtrl) then begin
          ...
          ((pt <> ptWHNotPurp) and (pt <> ptWHPapResult)) then begin
            X := LastX;
          end else begin
            if(Shared) and (assigned(FChildren)) and (FChildren.Count > 0) then begin
          ...
          if(MaxX > (PWidth - X - Gap)) then begin
            MaxX := PWidth - X - Gap;
          end;
          ...
          ((pt <> ptWHNotPurp) and (pt <> ptWHPapResult)) then begin
            ...
            if(assigned(Ctrl.Ctrl)) then begin
              if ScreenReaderSystemActive then begin
                if Supports(Ctrl.Ctrl, ICPRSDialogComponent) then begin
              ...
              if not Ctrl.Ctrl.Enabled then begin
              ...
              if(assigned(ud)) then begin
                SameLineCtrl.Add(ud);
                if(assigned(HelpBtn)) then begin
              ...
              end else if(HasVCombo) then begin
  ...
  begin
    if EnablePanel then begin
      if not ScreenReaderSystemActive then begin
      ...
      if ClearCB then begin
        cb := nil;
      end;
    end;

    if (FChecked and assigned(FPrompts) and (FPrompts.Count > 0)) then begin
...
begin  //TRemDlgElement.BuildControls()
  Result := nil;
  cb := nil;
  pnl := nil;
  AutoFocusControl := nil;
  X := TrueIndent;
  if(assigned(FPrompts)) then begin
    for i := 0 to FPrompts.Count-1 do begin
      TRemPrompt(FPrompts[i]).FCurrentControl := nil;
    end;
  end;
  if(ElemType = etDisplayOnly) then begin
    if(FText <> '') then begin
      inc(Y,Gap);
...
//    cb.Height := 17;
    ...
    for i := 0 to pnl.ControlCount - 1 do
      ...
      not (fsUnderline in TLabel(pnl.Controls[i]).Font.Style) then begin//If this isn't a hyperlink change then event handler
         TLabel(pnl.Controls[i]).OnClick := FieldPanelLabelOnClick;
      end;

    //cb.Enabled := Enabled;
    if(assigned(FParent) and (FParent.ChildrenRequired in [crOne, crNoneOrOne])) then begin
      cb.RadioStyle := TRUE;
    end;
...
//      end;
    ...
    if(FHasSharedPrompts) then begin
  ...
  if(assigned(AutoFocusControl)) then begin
    if(AutoFocusControl is TORComboBox) and
    (TORComboBox(AutoFocusControl).CheckBoxes) and
    (pos('1',TORComboBox(AutoFocusControl).CheckedString) = 0) then begin
      Result := AutoFocusControl
    end else if(TORExposedControl(AutoFocusControl).Text = '') then begin
      Result := AutoFocusControl
    end;
  end;
  if ScreenReaderSystemActive then begin
...
begin
  ...
  repeat
    ...
    if(i > 0) then begin
      ...
      if assigned(FldValues) and (CtrlID > 0) then begin
        j := FldValues.IndexOfPiece(IntToStr(CtrlID));
        if not(j<0) then begin
          if Fld.DateType in DateComboTypes then begin
      ...
      not ((Fld.FldType in [dftHyperlink, dftText]) and Fld.SepLines) then begin
...
begin
  ...
  try
    ...
    begin
      ilvl := IndentPNLevel;
      if(FPNText <> '') then begin
        txt := FPNText
      end else begin
        txt := FText;
        if not FReminder.FNoResolve then begin
          //If this is the CurrentReminderInDialog then we get the template field
          //values from the visual control in the dialog window.
          if FReminder = CurrentReminderInDialog then begin
            //kt original --> 5/16  txt := ResolveTemplateFields(txt, TRUE)
            txt := ResolveTemplateFields(txt, TRUE, false, false, false, '', '', TDBControlData(DBControlData));
            //kt 2/20/16 TO DO --> implement use of HTMLTxt := ResolveHTMLTemplateFields(txt, TRUE)
          end else begin
      ...
      if FReminder.FNoResolve then begin
      ...
      if(assigned(FPrompts)) then begin
        for i := 0 to FPrompts.Count-1 do begin
          Prompt := TRemPrompt(FPrompts[i]);
          if(not Prompt.FIsShared) then begin
            if Prompt.PromptType = ptMHTest then  WordWrap(Prompt.NoteText, Lst, ilvl, 4, true)
            else WordWrap(Prompt.NoteText, Lst, ilvl);
          end;
        end;
      end;
      if(assigned(FParent) and FParent.FHasSharedPrompts) then begin
        for i := 0 to FParent.FPrompts.Count-1 do begin
          Prompt := TRemPrompt(FParent.FPrompts[i]);
          if(Prompt.FIsShared) and (Prompt.FSharedChildren.IndexOf(Self) >= 0) then begin
            //AGP Change MH dll
            if (Prompt.PromptType = ptMHTest) then WordWrap(Prompt.NoteText, Lst, ilvl, 4, True)
            else WordWrap(Prompt.NoteText, Lst, ilvl);
          end;
    ...
    if (assigned(FChildren)) and (FChecked or (ElemType = etDisplayOnly)) then begin
      for i := 0 to FChildren.Count-1 do begin
        TRemDlgElement(FChildren[i]).AddText(Lst);
      end;
    end;
  finally
    if (not (FReminder is TReminder)) then begin
      ScootOver := 0;
    end;
  end;
end;
...
//  OK := ((ElemType <> etDisplayOnly) and FChecked);
  OK := FChecked;
  if(OK and Finishing) then begin
  ...
  if OK then begin
    if(assigned(FData)) then begin
      ...
      for i := 0 to FData.Count-1 do begin
        ...
        if assigned(RData.FActiveDates) then begin
          for j := 0 to (TRemData(FData[i]).FActiveDates.Count - 1) do begin
...
begin
  if(not FChildrenShareChecked) then begin
    FChildrenShareChecked := TRUE;
    if(ChildrenSharePrompts and assigned(FChildren)) then begin
      for i := 0 to FChildren.Count-1 do begin
...
//            if(Kid.ElemType <> etDisplayOnly) and (assigned(Kid.FPrompts)) then
            if(assigned(Kid.FPrompts)) then begin
              for j:= 0 to Kid.FPrompts.Count-1 do begin
          ...
          if(PList.Count > 1) then begin
            for i := 0 to PList.Count-2 do begin
              if(assigned(EList[i])) then begin
                FirstMatch := TRUE;
                Prompt := TRemPrompt(PList[i]);
                for j := i+1 to PList.Count-1 do begin
                  if(assigned(EList[j]) and
                  (Prompt.CanShare(TRemPrompt(PList[j])))) then begin
                    if(FirstMatch) then begin
                      FirstMatch := FALSE;
                      if(not assigned(FPrompts)) then begin
                        FPrompts := TList.Create;
                      end;
                      FHasSharedPrompts := TRUE;
                      Prompt.FIsShared := TRUE;
                      if(not assigned(Prompt.FSharedChildren)) then begin
                    ...
                    if(Kid.FHasComment) and (Kid.FCommentPrompt = PList[j]) then begin
      ...
      for i := 0 to FChildren.Count-1 do begin
        Kid := TRemDlgElement(FChildren[i]);
        if(assigned(Kid.FPrompts) and (Kid.FPrompts.Count = 0)) then begin
          Kid.FPrompts.Free;
          Kid.FPrompts := nil;
        end;
      end;
...
//  if(ElemType <> etDisplayOnly) and (FChecked) and (assigned(FPrompts)) then
  if(FChecked and (assigned(FPrompts))) then begin
    for i := 0 to FPrompts.Count-1 do begin
      Prompt := TRemPrompt(FPrompts[i]);
      ...
      ((pt in [ptVisitDate, ptVisitLocation]) and (Value = '0')))) then begin
        WordWrap('Element: ' + FText, List, 68, 6);
        txt := Prompt.ForcedCaption;
        if(pt = ptVisitDate) and Prompt.FMonthReq then begin
          txt := txt + ' (Month Required)';
        end;
        WordWrap('Item: ' + txt, List, 65, 6);
      end;
      if (Prompt.PromptOK and (not Prompt.Forced) and Prompt.Required and
      ((WHResultChk='') and (Value='')) and ((pt=ptWHPapResult) and (FData<>nil))) then begin
        WordWrap('Prompt: ' + Prompt.ForcedCaption, List, 65,6);
      end;
      if (Prompt.PromptOK and (not Prompt.Forced) and Prompt.Required
      and (pt=ptWHNotPurp)) and ((WHResultNot = '') and (Value = '')) then begin
        WordWrap('Element: ' + FText, List, 68, 6);
        WordWrap('Prompt: ' + Prompt.ForcedCaption, List, 65,6);
      end;
      ...
      (not Prompt.Forced) then begin
        if (Piece(Prompt.FData.FRec3,U,13) = '2') and (Prompt.FMHTestComplete = 0) then break;
        if (Pt = ptMHTest) and (Prompt.FMHTestComplete = 2) then begin
          if ((Prompt.FValue = '') or (pos('X',Prompt.FValue)>0)) then begin
            if Prompt.FValue = '' then begin
              WordWrap('MH test '+ Piece(Prompt.FData.FRec3,U,8) + ' not done',List,65,6);
            end;
            if pos('X',Prompt.FValue)>0 then begin
        ...
        if (Pt = ptMHTest) and (Prompt.FMHTestComplete = 0) or ((Prompt.FValue = '') and
        (Pos('New MH dll',Prompt.FValue) = 0)) then begin
          if Prompt.FValue = '' then begin
            WordWrap('MH test '+ Piece(Prompt.FData.FRec3,U,8) + ' not done',List,65,6);
          end;
          if pos('X',Prompt.FValue)>0 then begin
        ...
        if (Pt = ptMHTest) and (Prompt.FMHTestComplete = 0) and (Pos('New MH dll',Prompt.FValue) > 0) then begin
          WordWrap('MH test ' + Piece(Prompt.FData.FRec3, U, 8) + ' is not complete', List, 65, 6);
          WordWrap(' ',List,65,6);
        end;
        if (Pt = ptGAF) and ((Prompt.FValue = '0') or (Prompt.FValue = '')) then begin
  ...
  if (assigned(FChildren)) and (FChecked or (ElemType = etDisplayOnly)) then begin
    ...
    if(cReq in [crOne, crAtLeastOne, crAll]) then begin
      cnt := 0;
      for i := 0 to FChildren.Count-1 do begin
        Kid := TRemDlgElement(FChildren[i]);
//        if(Kid.FChecked and (Kid.ElemType <> etDisplayOnly)) then
      ...
      if(cReq = crOne) and (cnt <> 1) then begin
        msg := 'One selection required'
      end else if(cReq = crAtLeastOne) and (cnt < 1) then begin
        msg := 'One or more selections required'
      end else if (cReq = crAll) and (cnt < FChildren.Count) then begin
...
begin
  ...
  lastItem := 0;
  for c := 0 to Choices.Count - 1 do begin
    if (Prompt.CompareActiveDate(TStringList(ChoicesActiveDates[C]), encDt) = TRUE) then begin
      cnt := cnt + 1;
      lastItem := c;
      if (cnt>1) then break;
    end;
...
begin
  if(assigned(FParent)) then begin
    ...
    if(FParent.IndentChildrenInPN) then begin
      dec(Result,2);
    end;
  end else begin
    Result := 70;
...
begin
  if(FHasSubComments and FHasComment and assigned(FCommentPrompt)) then begin
    ok := FALSE;
    if(assigned(Sender)) then begin
      with (Sender as TORCheckBox) do begin
    ...
    if(ok) then begin
      txt := '';
      for i := 0 to FPrompts.Count-1 do begin
        with TRemPrompt(FPrompts[i]) do begin
          if(PromptType = ptSubComment) and (FValue = BOOLCHAR[TRUE]) then begin
            if(txt <> '') then begin
              txt := txt + ', ';
            end;
            txt := txt + Caption;
...
begin
  Inherited; //kt added 5/16
  DBControlData := nil; //kt 5/16 added
...
begin
  TmpSL := TStringList.Create;
  try
    for i := 0 to FFieldValues.Count-1 do begin
...
begin
  with TDlgFieldPanel(Sender) do begin
    Focus := TRUE;
    Invalidate;
    if Parent is TDlgFieldPanel then begin
...
var
  ...
  begin  //AddPrompt()
    ...
    if(pt = ptMST) then
    begin
      if (PCECat in MSTDataTypes) then begin
    ...
    end else if(PCECat = pdcVital) then begin
      if(pt = ptVitalEntry) then begin
    ...
    end else if(PCECat = pdcMH) then begin
      if(pt = ptMHTest) or (pt = ptGAF) then begin
        x := x + U + Prompt.GetValue
      end else begin
        exit;
      end;
    end else if(pt <> ptDataList) and (ord(pt) >= ord(low(TRemPromptType))) then begin
      Pdt := RemPromptTypes[pt];
      if (Pdt = dt) or (Pdt = dtAll) or
      ((Pdt = dtHistorical) and assigned(Prompt.FParent) and
      Prompt.FParent.Historical) then begin
        pnum := FinishPromptPieceNum[pt];
      end;
      if(pnum > 0) then begin
        if(pt = ptPrimaryDiag) then begin
  ...
  begin  //Add()
    ...
    begin
      ...
      begin
        for i := 0 to FParent.FPrompts.Count-1 do begin
          Prompt := TRemPrompt(FParent.FPrompts[i]);
          if(Prompt.PromptType = ptPrimaryDiag) then begin
    ...
    Skip := FALSE;
    if (PCECat = pdcMH) then begin
      IsGAF := Piece(FRec3, U, r3GAF);
      Value := FChoicePrompt.GetValue;
      if(Value = '') or ((IsGAF = '1') and (Value = '0')) then begin
        Skip := TRUE;
      end;
    end;

    if Finishing or (PCECat = pdcVital) then begin
      if(dt = dtOrder) then begin
        ...
        if(assigned(FParent)) then begin
          if(assigned(FParent.FPrompts)) then begin
            for i := 0 to FParent.FPrompts.Count-1 do begin
              Prompt := TRemPrompt(FParent.FPrompts[i]);
              if(not Prompt.FIsShared) then begin
          ...
          if(assigned(FParent.FParent) and FParent.FParent.FHasSharedPrompts) then begin
            for i := 0 to FParent.FParent.FPrompts.Count-1 do begin
              Prompt := TRemPrompt(FParent.FParent.FPrompts[i]);
              if(Prompt.FIsShared) and (Prompt.FSharedChildren.IndexOf(FParent) >= 0) then begin
                AddPrompt(Prompt, dt, x);
              end;
            end;
          end;
      ...
      Qty := 1;
      if(assigned(FParent) and assigned(FParent.FPrompts)) then begin
        if(PCECat = pdcProc) then begin
          for i := 0 to FParent.FPrompts.Count-1 do begin
            Prompt := TRemPrompt(FParent.FPrompts[i]);
            if(Prompt.PromptType = ptQuantity) then begin
      ...
      if (not Skip) then begin
        ...
        if(assigned(FParent) and FParent.Historical) then begin
          txt := txt + ' (Historical)';
        end;
        List.AddObject(txt, Self);
        inc(Result);
      end;
      if assigned(FParent) and assigned(FParent.FMSTPrompt) then begin
        txt := FParent.FMSTPrompt.Value;
        if txt <> '' then begin
          if FParent.FMSTPrompt.FMiscText = '' then begin
            i1 := 0;
            i2 := 2;
          end else begin
            i1 := 3;
          ...
          for i := i1 to i2 do begin
            if txt = MSTDescTxt[i,1] then begin
...
begin  //TRemData.AddData();
  Result := 0;
  if(assigned(FChoicePrompt)) and (assigned(FChoices)) then begin
    if not assigned(FChoicesActiveDates) then begin
      for i := 0 to FChoices.Count - 1 do begin
        if (copy(FChoicePrompt.GetValue, i+1, 1) = '1') then begin
          Add(FChoices[i], TRemPCERoot(FChoices.Objects[i]))
        end;
      end
    end else begin {if there are active dates for each choice then check them}
      if Self.FParent.Historical then begin
        EncDt := DateTimeToFMDateTime(Date)
      end else begin
        EncDt := RemForm.PCEObj.VisitDateTime;
      end;
      k := 0;
      for i := 0 to FChoices.Count - 1 do begin
        for j := 0 to (TStringList(Self.FChoicesActiveDates[i]).Count - 1) do begin
          ...
          if (EncDt >= ActDt) and (EncDt <= InActDt) then begin
            if(copy(FChoicePrompt.GetValue, k+1,1) = '1') then begin
              Add(FChoices[i], TRemPCERoot(FChoices.Objects[i]));
            end;
            inc(k);
...
begin
  if(assigned(FPCERoot)) then begin
    FPCERoot.Done(Self);
  end;
  if(assigned(FChoices)) then begin
    for i := 0 to FChoices.Count-1 do begin
      if(assigned(FChoices.Objects[i])) then begin
...
begin
  ...
  if(not FCaptionAssigned) then begin
...
begin
  ...
  if(Code = RemPromptCodes[ptVisitDate]) then begin
    if(copy(Result,1,1) = MonthReqCode) then begin
      FMonthReq := TRUE;
      delete(Result,1,1);
    end;
...
begin
  ...
  try
    ...
    try
      ...
      case pt of
        ...
        end; //ptPrimaryDiag, ptAdd2PL, ptContraindicated:

        ptVisitLocation: begin
          cbo := (Sender as TORComboBox);
          if(cbo.ItemIEN < 0) then begin
            NeedRedraw := (not cbo.DroppedDown)
          end else begin
            if(cbo.ItemIndex <= 0) then begin
              cbo.Items[0] := '0' + U + cbo.text;
            end;
            TmpValue := cbo.ItemID;
            if(StrToIntDef(TmpValue,0) = 0) then begin
              TmpValue := cbo.Text;
            end;
          end;
        end; //ptVisitLocation:
        ...
        ptWHPapResult: begin
          if (Sender is TWHCheckBox) then begin
            whCKB := (Sender as TWHCheckBox);
            if whCKB.Checked = true then begin
              ...
              for i := 0 to FParent.FData.Count-1 do begin
                if Piece(TRemData(FParent.FData[i]).FRec3,U,4)='WHR' then begin
        ...
        ptWHNotPurp: begin
          if (Sender is TWHCheckBox) then begin
            whCKB := (Sender as TWHCheckBox);
            if whCKB.Checked = true then begin
              if whCKB.Caption ='Letter' then begin
                if FParent.WHResultNot='' then begin
                  FParent.WHResultNot := 'L'
                end else if Pos('L',FParent.WHResultNot)=0 then begin
                  FParent.WHResultNot := FParent.WHResultNot +':L';
                end;
                if whCKB.FButton <> nil then whCKB.FButton.Enabled := true;
                if whCKB.FPrintNow <> nil then begin
                  whCKB.FPrintVis :='1';
                  whCKB.FPrintNow.Enabled := true;
                end;
              end;
              if whCKB.Caption ='In-Person' then begin
                if FParent.WHResultNot='' then begin
                  FParent.WHResultNot := 'I'
                end else if Pos('I',FParent.WHResultNot)=0 then begin
                  FParent.WHResultNot := FParent.WHResultNot+':I';
                end;
              end;
              if whCKB.Caption ='Phone Call' then begin
                if FParent.WHResultNot='' then begin
                  FParent.WHResultNot := 'P'
                end else if Pos('P',FParent.WHResultNot)=0 then begin
              ...
              if whCKB.Caption ='Letter' then begin
                for i:=1 to Length(WHValue) do begin
                  if WHValue1='' then begin
                    if (WHValue[i]<>'L') and (WHValue[i]<>':') then WHValue1 := WHValue[i];
                  end else if (WHValue[i]<>'L') and (WHValue[i]<>':') then WHValue1 := WHValue1+':'+WHValue[i];
                end;
                if (whCKB.FButton <> nil) and (whCKB.FButton.Enabled = true) then whCKB.FButton.Enabled := false;
                if (whCKB.FPrintNow <> nil) and (whCKB.FPrintNow.Enabled = true) then begin
                  whCKB.FPrintVis := '0';
              ...
              if whCKB.Caption ='In-Person' then begin
                for i:=1 to Length(WHValue) do begin
                  ...
                  end else if (WHValue[i]<>'I') and (WHValue[i]<>':') then WHValue1 := WHValue1+':'+WHValue[i];
                end;
              end;
              if whCKB.Caption ='Phone Call' then begin
                for i:=1 to Length(WHValue) do begin
                  ...
                  end else if (WHValue[i]<>'P') and (WHValue[i]<>':') then WHValue1 := WHValue1+':'+WHValue[i];
                end;
              end;
              FParent.WHResultNot := WHValue1;
            end;
          end else if ((Sender as TORCheckBox)<>nil) and (Piece(FRec4,U,12)='1') then begin
            ...
            (FParent.WHPrintDevice ='') then begin
              FParent.WHPrintDevice := SelectDevice(Self, Encounter.Location, false, 'Women Health Print Device Selection');
              FPrintNow :='1';
              if FParent.WHPrintDevice ='' then begin
                FPrintNow :='0';
                (Sender as TORCheckBox).Checked := false;
              end;
            end;
            if (((Sender as TORCheckBox).Caption = 'Print Now') and
            ((Sender as TORCheckBox).Enabled =true)) and ((Sender as TORCheckBox).Checked = false) then begin
              FParent.WHPrintDevice := '';
              FPrintNow :='0';
            end;
          end;
        ...
        ptSeries, ptReaction, ptLevelUnderstanding, ptSkinReading: begin//(AGP Change 26.1)
          TmpValue := (Sender as TORComboBox).ItemID;
        end; //ptExamResults, ... ptSkinReading:

        else if pt = ptVitalEntry then begin     //else of case statement
          ...
          if(idx > 0) then begin
            ...
            if (FParent.VitalDateTime = 0) and (TORExposedControl(Sender).Text <> '') then begin
              FParent.VitalDateTime := FMNow;
            end;
          end;
        end else if pt = ptDataList then begin
      ...
      if(TmpValue <> OrgValue) then begin
      ...
      end else if NeedRedraw then begin
        FParent.FReminder.BeginNeedRedraw;
        FParent.FReminder.EndNeedRedraw(Self);
      end;
    finally
  ...
  if(FParent.ElemType = etDisplayOnly) and (not assigned(FParent.FParent)) then begin
    RemindersInProcess.Notifier.Notify;
  end;
end;
...
begin  //TRemPrompt.PromptOK
  ...
  begin
    ...
    else if ( dt <> dtProcedure) and (dt <> dtDiagnosis) then begin
      Result := FALSE;
      if(assigned(FParent.FData)) then begin
        for i := 0 to FParent.FData.Count - 1 do begin
          if (TRemData(FParent.FData[i]).DataType = dt) then begin
            Result := TRUE;
            break;
          end;
        end;
    ...
    // historical values override the date check.
      ...
      if (Assigned(FParent.FData)) then begin
        for i := 0 to FParent.FData.Count - 1 do begin
          if (TRemData(FParent.FData[i]).DataType = dt) then begin
            if (FParent.Historical) then begin
              Result := TRUE;
              break;
            end else if (TRemData(FParent.FData[i]).FActiveDates <> nil) then begin
              encDate := TRemData(FParent.FData[i]).FParent.FReminder.FPCEDataObj.DateTime;
              if (RemDataActive(TRemData(FParent.FData[i]), encDate) = TRUE) then begin
                Result := TRUE;
              end;
              break;
            // else if (Assigned(TRemData(FParent.FData[i]).FChoices) and (TRemData(FParent.FData[i]).FChoices <> nil)) then
            end else if Assigned(TRemData(FParent.FData[i]).FChoices) then begin
              encDate := TRemData(FParent.FData[i]).FParent.FReminder.FPCEDataObj.DateTime;
              for C := 0 to TRemData(FParent.FData[i]).FChoices.Count - 1 do begin
                if (CompareActiveDate(TStringList(TRemData(FParent.FData[i]).FChoicesActiveDates[C]), encDate) = TRUE) then begin
                  Result := TRUE;
                  break;
                end;
              end;
...
begin
  ...
  if Add2PN then begin
    ...
    case pt of
      ...
      ptQuantity: begin
        if(StrToIntDef(tmp,1) <> 1) then Result := tmp;
      end;

     (* ptSkinReading: if(StrToIntDef(tmp,0) <> 0) then
                       Result := tmp;  *)

      ptSkinReading: begin  // (AGP Change 26.1)
        Result := tmp;
      end;
      ...
      ptVisitDate: begin
        try
          if(tmp <> '') and (tmp <> '0') and (length(Tmp) = 7) then begin
            dateStr := '';
            if FMonthReq and (copy(tmp,4,2) = '00') then Result := ''
            else begin
      ...
      ptWHNotPurp: begin
        if FParent.WHResultNot <> '' then begin
          ...
          if WHValue <> 'CPRS' then begin
            for cnt := 1 to Length(WHValue) do begin
              if Result ='' then begin
        ...
        end else if Forced = true then begin
          if pos(':',Piece(FRec4,U,6))=0 then begin
            if Piece(FRec4,U,6)='L' then begin
              Result := 'Letter';
              FParent.WHResultNot :='L';
            end;
            if Piece(FRec4,U,6)='I' then begin
              Result := 'In-Person';
              FParent.WHResultNot := 'I';
            end;
            if Piece(FRec4,U,6)='P' then begin
              Result := 'Phone Call';
              FParent.WHResultNot := 'P';
            end;
            if Piece(FRec4,U,6)='CPRS' then begin
              Result := '';
              FParent.WHResultNot := 'CPRS';
            end;
          end else begin
            WHValue := Piece(FRec4,U,6);
            for cnt := 0 to Length(WHValue) do begin
              if Result ='' then begin
                if WHValue[cnt]='L' then begin
                  Result := 'Letter';
                  FParent.WHResultNot := WHValue[cnt];
                end;
                if WHValue[cnt]='I' then begin
                  Result := 'In-Person';
                  FParent.WHResultNot := WHValue[cnt];
                end;
                if WHValue[cnt]='P' then begin
                  Result := 'Phone Call';
                  FParent.WHResultNot := WHValue[cnt];
                end;
              end else begin
                if (WHValue[cnt]='L')and(Pos('Letter',Result)=0) then begin
                  Result := Result +'; Letter';
                  FParent.WHResultNot := FParent.WHResultNot + ':' + WHValue[cnt];
                end;
                if (WHValue[cnt]='I')and(Pos('In-Person',Result)=0) then begin
                  Result := Result +'; In-Person';
                  FParent.WHResultNot := FParent.WHResultNot + ':' + WHValue[cnt];
                end;
                if (WHValue[cnt]='P')and(Pos('Phone Call',Result)=0) then begin
      ...
      ptExamResults, ptSkinResults, ptLevelSeverity,
      ptSeries, ptReaction, ptLevelUnderstanding: begin
        Result := tmp;
        if(Piece(Result,U,1) = '@') then begin
          Result := ''
        end else begin
          Result := GetPCEDisplayText(tmp, ComboPromptTags[pt]);
        end;
        ...
        if pt = ptDataList then begin
          if(assigned(FData) and assigned(FData.FChoices)) then begin
            if not(assigned(FData.FChoicesActiveDates)) then begin
              for i := 0 to FData.FChoices.Count - 1 do begin
                if(copy(tmp,i+1,1) = '1') then begin
                  if (Result <> '') then begin
                    Result := Result + ', ';
                  end;
              ...
              k := 0;
              for i := 0 to FData.FChoices.Count - 1 do begin
                for j := 0 to (TStringList(FData.FChoicesActiveDates[i]).Count - 1) do begin
                  ...
                  if (EncDt >= ActDt) and (EncDt <= InActDt) then begin
                    if(copy(tmp,k+1,1) = '1') then begin
                      if(Result <> '') then begin
                        Result := Result + ', ';
                      end;
                      Result := Result + Piece(FData.FChoices[i],U,12);
        ...
        end else if pt = ptMHTest then begin
          Result := FMiscText
        end else if (pt = ptGAF) and (MHDLLFound = false) then begin
          if(StrToIntDef(Piece(tmp, U, 1),0) <> 0) then begin
            Result := tmp;
          end
        end else if pt = ptMHTest then begin
          Result := FMiscText;
        ...
        *)
      //end;
      end;  //end of else of case
    end;  //case
  end;
  if(Result <> '') and (Caption <> '') then begin

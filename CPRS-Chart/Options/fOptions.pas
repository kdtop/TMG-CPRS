unit fOptions;
//kt 9/11 made changes to the FORM of this unit
 (*
...
type
  TfrmOptions = class(TfrmAutoSz)
    ...
    imgReport1: TImage;
    imgReport2: TImage;
    tsImages: TTabSheet;                                        //kt 9/11
    cbEnableImages: TCheckBox;                                  //kt 9/11
    lblImageRetvMethod: TLabel;                                 //kt 9/11
    cboTransMethod: TComboBox;                                  //kt 9/11
    lblDropBox: TLabel;                                         //kt 9/11
    editDropboxLocation: TEdit;                                 //kt 9/11
    btnBrowseDropBox: TBitBtn;                                  //kt 9/11
    cbEnableScanning: TCheckBox;                                //kt 9/11
    lblScannedImagesFolder: TLabel;                             //kt 9/11
    editScannedLocation: TEdit;                                 //kt 9/11
    btnBrowseScannedImages: TBitBtn;                            //kt 9/11
    lblPollingInterval: TLabel;                                 //kt 9/11
    edtPolFreq: TEdit;                                          //kt 9/11
    lblSeconds: TLabel;                                         //kt 9/11
    lblSelEditFor: TLabel;                                      //kt 9/11
    cboImageType: TComboBox;                                    //kt 9/11
    edtEditorName: TEdit;                                       //kt 9/11
    btnPickEditor: TBitBtn;                                     //kt 9/11
    btnDelType: TSpeedButton;                                   //kt 9/11
    lblStockImgFolder: TLabel;                                  //kt 9/11
    editStockLocation: TEdit;                                   //kt 9/11
    btnBrowseStockImages: TBitBtn;                              //kt 9/11
    ...
    procedure btnBrowseStockImagesClick(Sender: TObject);       //kt 9/11
    procedure btnDelTypeClick(Sender: TObject);                 //kt 9/11
    procedure btnPickEditorClick(Sender: TObject);              //kt 9/11
    procedure cboImageTypeChange(Sender: TObject);              //kt 9/11
    procedure edtPolFreqChange(Sender: TObject);                //kt 9/11
    procedure btnBrowseScannedImagesClick(Sender: TObject);     //kt 9/11
    procedure editScannedLocationChange(Sender: TObject);       //kt 9/11
    procedure cbEnableScanningClick(Sender: TObject);           //kt 9/11
    procedure btnBrowseDropBoxClick(Sender: TObject);           //kt 9/11
    procedure cbEnableImagesClick(Sender: TObject);             //kt 9/11
    procedure cboTransMethodChange(Sender: TObject);            //kt 9/11
    procedure editDropboxLocationChange(Sender: TObject);       //kt 9/11
    procedure tsImagesShow(Sender: TObject);                    //kt 9/11
  ...
  private
    ...
    FdirtyImageSettings: boolean;  // used to determine edit changes to Image Settings  //kt 9/11
    ...
    FsortCol: integer;
    OptionsIniFile: TIniFile; //kt 9/11
    FsortAscending: boolean;
    FLastClickedItem: TListItem;
    OpenFileDialog: TOpenDialog;  //kt 9/11
    ...
    function BrowseDialog(const Title: string; const Flag: integer): string;  //kt
    function ExtractFileType (tempStr: string): string; //kt
    procedure SaveAndReloadImageType; //kt
    Procedure LoadImageTypes;  //kt
    function ImageTypesStr(ExcludeStr : string='') : string; //kt
  public
    { Public declarations }
  end;
...
uses fOptionsDays, fOptionsReminders, fOptionsSurrogate,
     ...
     ShellAPI,               //kt 9/11
     uTMGOptions,            //kt 9/11
     fUploadImages, fImages, fNotes, //kt 9/11
     ShlObj,                 //kt 9/11
     ORNet;                  //kt 9/11 for access to RPCBrokerV

const
  ADD_NEW_TYPES = '<Add New Type>';
...
begin
  ...
  //  on server.  For those server-stored settings, use uTMGOptions
  OpenFileDialog := TOpenDialog.Create(self);
  OpenFileDialog.InitialDir := 'C:\';
  OpenFileDialog.Filter := 'Executable|*.exe';
  OptionsIniFile := TIniFile.Create(ChangeFileExt(Application.ExeName, '.INI'));
  iniFileTypes := OptionsIniFile.ReadString('IMAGE_EDIT','FILE_TYPES','');
  i := 1;
  FileType := piece(iniFileTypes,'^',i);
  while FileType <> '' do begin
    cboImageType.AddItem(FileType,Self);
    i := i + 1;
    FileType := piece(iniFileTypes,'^',i);
  end;
  cboImageType.AddItem(ADD_NEW_TYPES,Self);
  //kt 9/11 end mod ------

  LoadNotifications;
  LoadOrderChecks;
  LoadImageSettings;            //kt 9/11
  FdirtyImageSettings := false; //kt 9/11
...
var
  i: integer;
begin
    OptionsIniFile.Free;  //kt 9/11
    OpenFileDialog.free;  //kt 9/11
...
begin
  inherited;
  s := lblAddSigPic.Caption;
  SetPiece(s,':',2,' '+User.Name);
  lblAddSigPic.Caption := s;
end;
...
begin
  if FdirtyNotifications then
    ApplyNotifications;
  if FdirtyImageSettings then  //kt 9/11
      ApplyImageSettings;      //kt 9/11
...
begin
  ...
  procedure TfrmOptions.ApplyImageSettings;    //kt added entire function    9/11
  begin
    ...
    //frmImageUpload.FScanDir := editScannedLocation.text;

    uTMGOptions.WriteString('Stock Directory',editStockLocation.text);

    uTMGOptions.WriteInteger('Pol Interval (milliseconds)',StrToInt(edtPolFreq.text));
    //frmImageUpload.PolInterval := StrToInt(edtPolFreq.text);
...
begin
   ...
   editStockLocation.text := uTMGOptions.ReadString('Stock Directory','');
...
procedure TfrmOptions.cboImageTypeChange(Sender: TObject);
//kt 9/11 added
var Ext,Name : string;
begin
  //OptionsIniFile.WriteString('IMAGE_EDIT','FILE_TYPES','');
  btnPickEditor.Enabled := True;
  if cboImageType.Text = ADD_NEW_TYPES then begin
    Ext := LowerCase(InputBox('Enter a new file type.','Enter a new file extension (e.g. ''.jpeg'')', ''));
    if Ext = '' then exit;
    if RightStr(Ext,1) <> '.' then Ext := Ext + '.';
    Name := LowerCase(InputBox('Enter NAME of new file type.','Enter a short description new extension (e.g. ''JPEG image'')', ''));
    If Name = '' then exit;
    //cboImageType.AddItem(InputBox('Enter a new file type.', 'Enter the name with extension in parenthesis e.g. jpeg(.jpg)', ''),Self);
    cboImageType.AddItem( Name+'('+Ext+')' ,Self);
    SaveAndReloadImageType;
  end;
  if cboImageType.Text <> '' then
    edtEditorName.Text := OptionsIniFile.ReadString('IMAGE_EDIT',ExtractFileType(cboImageType.Text),'');
end;


procedure TfrmOptions.btnPickEditorClick(Sender: TObject);
//kt 9/11 added
//var SelectedEditor: string;
begin
  if OpenFileDialog.Execute then begin
    edtEditorName.text := OpenFileDialog.FileName;
    OptionsIniFile.WriteString('IMAGE_EDIT',ExtractFileType(cboImageType.Text),edtEditorName.text);
  end;
end;
...
var  newTypes : string;
     DelTypeName : string;
begin
  if cboImageType.Text = ADD_NEW_TYPES then exit;
  if MessageDlg('Delete: '+cboImageType.Text,mtConfirmation,mbOKCancel,0) <> mrOK then exit;
  DelTypeName := ExtractFileType(cboImageType.Text);
  newTypes := ImageTypesStr(cboImageType.Text);
  OptionsIniFile.WriteString('IMAGE_EDIT','FILE_TYPES',newTypes);
  LoadImageTypes;
  OptionsIniFile.DeleteKey('IMAGE_EDIT',DelTypeName);
end;


procedure TfrmOptions.btnBrowseStockImagesClick(Sender: TObject);
//kt 9/11 added
var
   sFolder: string;
begin
  sFolder := BrowseDialog('Choose a Stock images folder', BIF_RETURNONLYFSDIRS);
  if sFolder <> '' then begin
    if rightstr(sFolder,1) = '\' then begin
       editStockLocation.text := sFolder;
    end else begin
       editStockLocation.text := sFolder + '\';
    end;
  end;
...
procedure TfrmOptions.SaveAndReloadImageType;
//kt 9/11 added
var tempText : string;
begin
  tempText := ImageTypesStr();
  OptionsIniFile.WriteString('IMAGE_EDIT','FILE_TYPES',tempText);
  LoadImageTypes;
end;

Procedure TfrmOptions.LoadImageTypes;
//kt 9/11 added
var tempText,FileType : string;
    i : Integer;
begin
  cboImageType.Clear;
  tempText := OptionsIniFile.ReadString('IMAGE_EDIT','FILE_TYPES','');
  i := 1;
  repeat
    FileType := piece(tempText,'^',i); Inc(i);
...
function TfrmOptions.ImageTypesStr(ExcludeStr : string) : string;
//kt 9/11 added
var i: integer;
    oneType : string;
begin
  Result := '';
  for i := 0 to cboImageType.Items.Count - 1 do begin
    oneType := cboImageType.Items.Strings[i];
    if oneType = ADD_NEW_TYPES then continue;
    if oneType = ExcludeStr then continue;
    Result := Result + oneType + '^'
  end;
end;


function TfrmOptions.ExtractFileType (tempStr: string): string;
//kt 9/11 added
//var
//   i : integer;
begin
  ...
  //i := pos('(',tempString);
  //tempString := Rightstr(tempString,StrLen(PChar(tempString))-i);
  //i := pos(')',tempString);
  //result := LeftStr(tempString,i-1);
end;

procedure TfrmOptions.tsImagesShow(Sender: TObject);
//kt 9/11 added
var
   i : integer;
   frmImageUpload: TfrmImageUpload;  //kt 9/25/15
begin
  frmImageUpload := TfrmImageUpload.Create(Self);   //kt 9/25/15
//When this tab is shown, check fUploadImages to see if an image there has been selected
//if so, load the file extension into the ImageType combo box
  if assigned (frmImageUpload) then begin
     for i := 0 to cboImageType.Items.count - 1 do begin
       if PosEx(Uppercase(frmImageUpload.FileExt),Uppercase(cboImageType.Items.Strings[i])) > 0 then begin
          cboImageType.ItemIndex := i;
          cboImageTypeChange(self);
       end;
     end;
  end;
  frmImageUpload.Free; //kt 9/25/15
end;

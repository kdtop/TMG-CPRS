...
type
  TfrmRemDlg = class(TfrmBase508Form)
    ...
    btnFinish: TButton;
    btnCancel: TButton;
    procedure SBMouseWheel(Sender: TObject; Shift: TShiftState; WheelDelta: Integer; MousePos: TPoint;
      var Handled: Boolean);
  ...
  private
    ...
    FSilent: boolean;
    FDBControlData : TDBControlData; //kt added 5/16  Owned here. 
  protected
...
begin
  ...
  try
    ...
    if (InitDlg and (not assigned(frmRemDlg))) then begin
      ...
      //kt doc: Below is the only place frmRemDlg is instantiated.  Released in KillReminderDialog()
...
begin
  ...
  if (FReminder <> Rem) then begin
    if (assigned(FReminder)) then begin
      Abort := FALSE;
      Flds := FALSE;
      TmpList := TStringList.Create;
      try
        FReminder.FinishProblems(TmpList, Flds);
        if (TmpList.Count > 0) or Flds then begin
  ...
  UpdateButtons;
  Show;
  Application.ProcessMessages; //kt added 7/16/15
end;
...
begin
  ...
  RemForm.Form := nil;
  //kt begin mod 5/16 -----------------------
  IEN8925 := 0;  //TO DO, GET TARGET DOCUMENT??...
...
begin
  box := GetBox(TRUE);
  if RectContains(box.BoundsRect, box.ScreenToClient(MousePos)) then begin
  ...
  begin
    for i := Ctrl.ControlCount-1 downto 0 do begin
      if (Ctrl.Controls[i].Owner = Self) then begin
        ...
        {
        //kt begin mod ---------
        //NOTE: I made this mod when tracking down a problem that ended up being elsewhere
        ChildCtrl := Ctrl.Controls[i];
        ChildCtrl.Parent := nil;
        //Above, in turn, effects call of Ctrl.RemoveControl(ChildCtrl) ...  I think
        ChildCtrl.Free;
        //kt end mod ------------ 7/12/16
        }
      end;
...
begin
  ...
  if idx > -1 then begin
    ...
    (TReminderDialog(TReminder(RemindersInProcess.Objects[idx])).MHTestArray.Count > 0) then begin
      for j := 0 to TReminderDialog(TReminder(RemindersInProcess.Objects[idx])).MHTestArray.Count - 1 do begin
    ...
    if Assigned(TReminderDialog(TReminder(RemindersInProcess.Objects[idx])).MHTestArray) then begin
  ...
  if (MHKillArray.Count > 0) and (RemindersInProcess.Count > 1) then begin
    for I := 0 to RemindersInProcess.Count - 1 do begin
      ...
      (TReminderDialog(TReminder(RemindersInProcess.Objects[i])).MHTestArray.Count > 0) then begin
        for j := 0 to TReminderDialog(TReminder(RemindersInProcess.Objects[i])).MHTestArray.Count - 1 do begin
  ...
  if MHKillArray.Count > 0 then begin
    for I := 0 to MHKillArray.Count - 1 do begin
...
var
  ...
  begin
    ...
    if (assigned(Ctrl) and IsOnBox(Ctrl)) then begin
      if (Active is TORCheckBox) then begin
        LastCB := Active.Tag;
      end;
      if (LastCB = 0) then begin
        ...
        for i := 0 to ComponentCount-1 do begin
          if (IsOnBox(Components[i])) then begin
            ...
            if (Ctrl is TORCheckBox) and (Ctrl.Tag <> 0) then begin
              CtrlIdx := i;
            end;
...
begin //TRemDlgElement.BuildControls()
  if (assigned(FReminder)) then begin
    Box := GetBox(TRUE);
    if Box.ControlCount > 0 then ClearControls; //AGP Change 26.1 this change should resolve the problem with Duplicate CheckBoxes appearing on some reminder dialogs CQ #2843
...
end; //TRemDlgElement.BuildControls()

function TfrmRemDlg.GetBox(Other: boolean = FALSE): TScrollBox;
begin
  //kt doc --- truth table -------------------------------
  //  FUseBox2 = true,   Other = true -->  false --> sb1
  //  FUseBox2 = true,   Other = false --> true  --> sb2
  //  FUseBox2 = false,  Other = true -->  true  --> sb2
  //  FUseBox2 = false,  Other = false --> false --> sb1
  //kt doc end --------------------------------------------
  if (FUseBox2 xor Other) then begin
    Result := sb2
  end else begin
    Result := sb1;
  end;
...
begin
  if uRemDlgStarting then exit; //kt added.  I was getting weird re-enterant problems... 7/16
          ...
          try
            ...
            if HTMLEditing then begin  //kt added this entire block                      //kt 9/11
              if (not FProcessingTemplate) and (not InsertRemTextAtCursor) then begin    //kt 9/11
                RemForm.NewNoteHTMLE.MoveCaretToEnd;                                     //kt 9/11
              end;                                                                       //kt 9/11
              AddLine := TRUE;                                                           //kt 9/11
            end else begin             //kt 9/11 (original code below)
            ...
            if (TmpText.Count > 0) then begin
              if not FProcessingTemplate then begin
                tmp := ClinRemText;
                //kt 9/11 if (tmp <> '') then begin
                if (tmp <> '') and (not HTMLEditing) then begin  //kt 9/11
              ...
              if AddLine then
                TmpText.Insert(0, '');
              //kt original --> CheckBoilerplate4Fields(TmpText, 'Unresolved template fields from processed Reminder Dialog(s)');
              CheckBoilerplate4Fields(TmpText, 'Unresolved template fields from processed Reminder Dialog(s)',[], FDBControlData); //kt
              if TmpText.Count = 0 then begin
                Process := FALSE
              end else begin
            ...
            //kt 9/11 if (Process) then begin
            if (Process) and (not HTMLEditing) then begin  //kt 9/11
        ...
        if (Process) then begin
          ...
          try
              ...
              try
                ...
                try
                  ...
                  try
                        ...
                        if (TmpData.Count > 0) then begin
                          ...
                          for i := 0 to TmpData.Count-1 do begin
                            ...
                            case Cat of
                              ...
                              pdcVital: if (StoreVitals) then begin
                    ...
                    for i := 0 to MSTList.Count-1 do begin
                ...
                if (MHList.Count > 0) then begin
                  ...
                  end; {for i := 0 to MHList.Count-1 }
                end; {if (MHList.Count > 0) }
              finally
                MHList.Free;
            ...
            if (OrderList.Count > 0) then begin
              ...
              repeat
                Done := TRUE;
                if not ReadyForNewOrder(DelayEvent) then begin
...
procedure TfrmRemDlg.SBMouseWheel(Sender: TObject; Shift: TShiftState; WheelDelta: Integer;
                                  MousePos: TPoint; var Handled: Boolean);
//kt 5/15 added function, AND link to this function from OnMouseWheel event from sb2 scroll box on frmRemDlg
var
  i: Integer;
begin
  inherited;
  //MessageDlg('Here!', mtInformation, [mbOK],0);
  Handled := PtInRect(sb2.ClientRect, sb2.ScreenToClient(MousePos));
  if Handled then for i := 1 to Mouse.WheelScrollLines do begin
    try
      if WheelDelta > 0 then begin
        sb2.Perform(WM_VSCROLL, SB_LINEUP, 0)
      end else begin
        sb2.Perform(WM_VSCROLL, SB_LINEDOWN, 0);
      end;
    finally
      sb2.Perform(WM_VSCROLL, SB_ENDSCROLL, 0);
    end;
  end;
end;

unit fMultiHandleLoose;

//kt Added entire form 12/6/22

interface

uses
  Windows, Messages, SysUtils, DateUtils, Variants, Classes, Graphics, Controls, Forms,
  Dialogs, ComCtrls, rCore, fFrame, fAddlSigners, Math,fImagePatientPhotoID, fNotes, uCore, uDocTree,
...
type
  ...
  TfrmMultiHandleLoose = class(TForm)
    pnlTop: TPanel;
    pnlBottom: TPanel;
    pnlLeft: TPanel;
    Splitter1: TSplitter;
...
function HandleMultipleLooseDocs() : boolean;
...
var
  frmMultiHandleLoose: TfrmMultiHandleLoose;
  InitialBounds:string;
  InitPnlLeft,InitPnlCenter,InitZoom : string;
begin
  result := false;
  InitialBounds := sCallV('ORWCH LOADSIZ',['frmMultiHandleLoose']);
  InitPnlLeft := sCallV('ORWCH LOADSIZ',['frmMultiHandleLoose.pnlLeft']);
  InitPnlCenter := sCallV('ORWCH LOADSIZ',['frmMultiHandleLoose.pnlCenter']);
  InitZoom := sCallV('ORWCH LOADSIZ',['frmMultiHandleLoose.ZoomValue']);
  frmMultiHandleLoose := TfrmMultiHandleLoose.Create(Application);
  try
    frmMultiHandleLoose.Initialize();
    if InitialBounds<>'' then begin
      frmMultiHandleLoose.Left := strtoint(piece(InitialBounds,',',1));
      frmMultiHandleLoose.Top := strtoint(piece(InitialBounds,',',2));
      frmMultiHandleLoose.Width := strtoint(piece(InitialBounds,',',3));
      frmMultiHandleLoose.Height := strtoint(piece(InitialBounds,',',4));
    end;
    if InitPnlLeft<>'' then
      frmMultiHandleLoose.pnlLeft.Width := strtoint(piece(InitPnlLeft,',',1));
    if InitPnlCenter<>'' then
      frmMultiHandleLoose.pnlCenter.Width := strtoint(piece(InitPnlCenter,',',1));
    if InitZoom<>'' then begin
      ...
      frmMultiHandleLoose.FInitZoomValue := strtoint(piece(InitZoom,',',1));
    end else begin
      frmMultiHandleLoose.FInitZoomValue := -1;
    end;
    if frmMultiHandleLoose.ShowModal = mrOK then begin
      result := true;
    end;
  finally
    frmMultiHandleLoose.Free;
  end;
end;
...
procedure TfrmMultiHandleLoose.FormClose(Sender: TObject; var Action: TCloseAction);
...
begin
   ...
   SaveResults := sCallV('ORWCH SAVESIZ',['frmMultiHandleLoose',Bounds]);
   Bounds := inttostr(pnlLeft.Width)+',0,0,0';
   SaveResults := sCallV('ORWCH SAVESIZ',['frmMultiHandleLoose.pnlLeft',Bounds]);
   Bounds := inttostr(pnlCenter.Width)+',0,0,0';
   SaveResults := sCallV('ORWCH SAVESIZ',['frmMultiHandleLoose.pnlCenter',Bounds]);
   Bounds := inttostr(FZoomValue)+',0,0,0';
   SaveResults := sCallV('ORWCH SAVESIZ',['frmMultiHandleLoose.ZoomValue',Bounds]);
end;

procedure TfrmMultiHandleLoose.FormCreate(Sender: TObject);
begin
...
procedure TfrmMultiHandleLoose.FormDestroy(Sender: TObject);
var i : integer;
begin
...
procedure TfrmMultiHandleLoose.FormShow(Sender: TObject);
begin
...
procedure TfrmMultiHandleLoose.Initialize();
...
var
  ...
  tmpList : TStringList;
begin
  //SetupInfoList(Items);  <-- Don't need any further
  tmpList := TStringList.Create();
  ListNotesForTree(tmpList, NC_SCANNED_LOOSE_DOCS, 0, 0, 0, 0, True);
  //NewItem := nil;
  if tmpList.Count > 0 then begin
    CreateListItemsforDocumentTree(FDocList, tmpList, NC_SCANNED_LOOSE_DOCS, '', True, 0);
    frmNotes.UpdateTreeView(FDocList, lvUnSelected);
  end;
    FDocList.Clear;
...
procedure TfrmMultiHandleLoose.SetupInfoList(Items : TListItems);
//NOTE: Items contents should NOT BE MODIFIED!  They are owned and managed by sender.
var
  i,j : integer;
...
procedure TfrmMultiHandleLoose.btnAddToSignClick(Sender: TObject);
var ItemInfo : TItemInfo;
    Success : boolean;
begin
  //ItemInfo := SelectedItemInfoFromLB(lvUnSelected);
  // FIX ItemInfo := SelectedItemInfoFromLV(lvUnSelected);
...
procedure TfrmMultiHandleLoose.TransferCurrentFromUnselectedToActionList();
var j : integer;
begin
  WebBrowser.Navigate('about:blank');
  //FIX j := lvUnSelected.ItemIndex;
  //FIX MoveItemBetweenLists(j, lvUnSelected, lbSelected);
  while j >= lvUnSelected.Items.Count do dec(j);
  //FIX lvUnSelected.ItemIndex := j;
...
procedure TfrmMultiHandleLoose.IdentifyAdditionalSigners(ItemInfo : TItemInfo);
...
procedure TfrmMultiHandleLoose.btnRemoveClick(Sender: TObject);
var j : integer;
    ItemInfo : TItemInfo;
begin
  ...
  //FIX MoveItemBetweenLists(j, lbSelected, lvUnSelected);
  lvUnSelected.AlphaSort;
  //FIX lvUnSelected.ItemIndex := lvUnSelected.Items.Count-1;
  lvUnSelected.SetFocus;
end;


procedure TfrmMultiHandleLoose.MoveItemBetweenLists(j : integer; CurLB:TListBox; TargetLB : TListView);
var s,Date : string;
    i : integer;
    NewItem : TListItem;
begin
...
procedure TfrmMultiHandleLoose.MoveItemBetweenLists(j : integer; CurLB:TListView; TargetLB : TListBox);
var s : string;
    Item : TListItem;
    i : integer;
begin
...
procedure TfrmMultiHandleLoose.btnCancelClick(Sender: TObject);
begin
...
procedure TfrmMultiHandleLoose.btnDeleteClick(Sender: TObject);
var ItemInfo : TItemInfo;
    Title : string;
    Suggestion : string;
begin
...
procedure TfrmMultiHandleLoose.btnEditNormalZoomClick(Sender: TObject);
begin
  ZoomReset;  //handle restore size
end;

procedure TfrmMultiHandleLoose.btnEditZoomInClick(Sender: TObject);
begin
  ZoomIn;  //handle enlarge size
end;

procedure TfrmMultiHandleLoose.btnEditZoomOutClick(Sender: TObject);
begin
  ZoomOut;  //handle shrink size
end;

procedure TfrmMultiHandleLoose.btnMoveToLooseClick(Sender: TObject);
var //j : integer;
    ItemInfo : TItemInfo;
    //Success : boolean;
...
function TfrmMultiHandleLoose.ProcessMoveToLoose(ItemInfo : TItemInfo):boolean;
var Success : boolean;
    ActionSts,DeleteSts : TActionRec;
    ImageList:TList;
    i : integer;
...
procedure TfrmMultiHandleLoose.btnNextClick(Sender: TObject);
//NOTE: disabled unless next is possible.
begin
  WebBrowser.Navigate('about:blank');
  //FIX lvUnSelected.ItemIndex := lvUnSelected.ItemIndex + 1;
  lvUnSelected.SetFocus;
  lbUnSelectedClick(Sender);
end;

procedure TfrmMultiHandleLoose.btnPrevClick(Sender: TObject);
...
procedure TfrmMultiHandleLoose.lbSelectedClick(Sender: TObject);
begin
  DisplayFromList(lbSelected);
  //FIX lvUnSelected.ItemIndex := -1;
  UpdateButtonEnableStates;
end;

procedure TfrmMultiHandleLoose.lbUnSelectedClick(Sender : TObject);
begin
  //FIX DisplayFromListView(lvUnSelected);
  lbSelected.ItemIndex := -1;
  UpdateButtonEnableStates;
end;

procedure TfrmMultiHandleLoose.DisplayFromList(LB : TListBox);
var
  ItemInfo : TItemInfo;
begin
  SelectedDFN := '';
...
procedure TfrmMultiHandleLoose.DisplayFromListView(LV : TListView);
var
  ItemInfo : TItemInfo;
begin
  SelectedDFN := '';
...
function TfrmMultiHandleLoose.SelectedItemInfoFromLB(LB : TListBox) : TItemInfo;
var
  i, DataIndex : integer;
begin
  Result := nil;  //default
...
function TfrmMultiHandleLoose.SelectedItemInfoFromLV(LV : TListView) : TItemInfo;
var
  i, DataIndex : integer;
begin
  Result := nil;  //default
...
function TfrmMultiHandleLoose.PatientDisplayName(ItemInfo : TItemInfo) : string;
begin
  //Could enhance later
...
procedure TfrmMultiHandleLoose.PatientImageMouseEnter(Sender: TObject);
//var //refresh : boolean;
    //ItemInfo : TItemInfo;
begin
  //if not assigned(ItemInfo) then exit;
...
procedure TfrmMultiHandleLoose.PatientImageMouseLeave(Sender: TObject);
begin
  inherited;
...
procedure TfrmMultiHandleLoose.pnlCenterBottomResize(Sender: TObject);
...
procedure TfrmMultiHandleLoose.pnlRightCanResize(Sender: TObject; var NewWidth, NewHeight: Integer; var Resize: Boolean);
begin
...
procedure TfrmMultiHandleLoose.LoadNoteForView(ItemInfo : TItemInfo);
...
procedure TfrmMultiHandleLoose.lvUnSelectedChange(Sender: TObject; Item: TListItem; Change: TItemChange);
begin
  lvUnSelectedClick(Sender);
end;

procedure TfrmMultiHandleLoose.lvUnSelectedClick(Sender: TObject);
begin
  //FIX DisplayFromListView(lvUnSelected);
  UpdateButtonEnableStates;
end;

procedure TfrmMultiHandleLoose.lvUnSelectedColumnClick(Sender: TObject; Column: TListColumn);
begin
...
procedure TfrmMultiHandleLoose.lvUnSelectedCompare(Sender: TObject; Item1, Item2: TListItem; Data: Integer;
  var Compare: Integer);
begin
...
procedure TfrmMultiHandleLoose.lvUnSelectedResize(Sender: TObject);
begin
  //lvUnSelected.Column[2].width := lvUnSelected.Width - 175;
end;

procedure TfrmMultiHandleLoose.UpdateButtonEnableStates;
begin
  {   FIX
...
function TfrmMultiHandleLoose.UniqueFilename(FolderPath : string; FileSuffix : string) : string;
...
procedure TfrmMultiHandleLoose.btnSignAllClick(Sender: TObject);
...
function TfrmMultiHandleLoose.ProcessOne(ItemInfo : TItemInfo; ESCode : string) : boolean;  //result TRUE if successful sign.
...
function TfrmMultiHandleLoose.DeleteOneTIU(ItemInfo : TItemInfo): boolean;  //result TRUE if note deleted
...
begin
    ...
    x := sCallV('TIU DELETE RECORD', [ItemInfo.intIEN8925, 'Deleted via frmMultiHandleLoose']);
    result := Piece(x, U, 1) = '0';
end;

function TfrmMultiHandleLoose.SignOneTIU(ItemInfo : TItemInfo; ESCode : string) : boolean;  //result TRUE if successful sign.
const
...
function TfrmMultiHandleLoose.AllowSignature(ItemInfo : TItemInfo): Boolean;
...
procedure TfrmMultiHandleLoose.ZoomReset;
//copied and modified from TMGHTML2
begin
  SetZoom(100);
end;

procedure TfrmMultiHandleLoose.ZoomIn;
//copied and modified from TMGHTML2
begin
  SetZoom(FZoomValue + FZoomStep);
end;

procedure TfrmMultiHandleLoose.ZoomOut;
//copied and modified from TMGHTML2
begin
  SetZoom(FZoomValue - FZoomStep);
end;
...
procedure TfrmMultiHandleLoose.SetZoom(Pct : integer);
//copied and modified from TMGHTML2
var

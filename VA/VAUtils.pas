...
//Name means Piece-Not-Case-Sensitive, meaning match for Delim is not case sensitive.
var tempS : string;
begin
...
type  //kt 1/16 added type block
  TStartStop = Record
    Start, Stop, Len : Integer;
  end;
  TSSArray = array of TStartStop;

procedure MapPieces(const  S : string; Delim : string; MaxPiece : integer; SSArr : TSSArray);
//kt 1/16 Added entire function
var
  i : integer;
  Done : boolean;
  p1,p2,DelLen : integer;
begin
  DelLen := length(Delim);
  Done := false;
  p1 := 1;
  for i := 1 to MaxPiece do begin
    if Done then begin
      SSArr[i-1].Start := -1;
      SSArr[i-1].Stop := -1;
      SSArr[i-1].Len := 0;
...
var SSArr : TSSArray;
begin
  SetLength(SSArr, PieceNum);
  MapPieces(S, Delim, PieceNum, SSArr);
  Result := MidStr(S, SSArr[PieceNum-1].Start, SSArr[PieceNum-1].Len);
  Finalize(SSArr);
end;

function Piece_OLD(const S: string; Delim: string; PieceNum: Integer): string; overload;
//kt 9/11 Added entire function
//kt 1/2016 fix!
{This function fails on this input:
  S='<DATE initial="1/2/1980">'
  Delim='>'
  PieceNum=2
}
...
function Pieces2(const S: string; Delim: string; PieceStart,PieceEnd: Integer): string; overload;
//kt 9/11 Added entire function
var Remainder : String;
    PieceNum : integer;
    PieceLen,p : integer;
...
//kt 9/11 Added entire function
//Name means Pieces-Not-Case-Sensitive, meaning match for Delim is not case sensitive.
var tempS : string;
begin
  tempS := AnsiReplaceText(S,Delim,UpperCase(Delim));

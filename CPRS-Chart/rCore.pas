...
type
  ...
  TPtIDInfo = record                              // record for ORWPT IDINFO
    ...
    Location: string;
    RoomBed: string;
    //ADD VWPT BELOW FOR HRN  //kt 9/11 ported
    HRN : string;
    AltHRN :string;
    DueColor: TColor;  //kt 10/23/14
    DueHint: string;  //kt 10/23/14
  end;
  ...
  TPtSelect = record                              // record for ORWPT SELECT
    ...
    Attending: string;
    Associate: string;
    //ADD VWPT BELOW FOR HRN AltHRN  //kt 9/11
    HRN : string;
    AltHRN : string;
    InProvider: string;
...
procedure SubSetOfTMGProviders(Dest: TStrings);  //ELH ADDED
...
procedure ListPtByAppt(Dest: TStrings; Provider:integer; FirstDt, LastDt: string); //TFMDateTime);   //elh added
...
function GetPatientDueInfo(DueInfo: TStringList; DFN: string; var ToolText: string; DefaultColor:TColor = clInfoBK):TColor;  //kt 10/23/14
...
uses
  Hash, uCore, ShlObj, Windows
  , StrUtils, uTMGUtil, uTMGOptions  //kt 1015
  ;
...
//vwpt code 4/17/0  see below
  ...
  //end vwpt
begin
  //agp wv begin change  //kt changed slightly
  if hidePtSSN = true then begin
    Result := x;
    exit;
  end;
  //agp wv end change
  //kt 9/11 -- Start Mod ----
  Result := AnsiReplaceStr(x,'-','');  //kt 6/9/10 -- fix when SSN passed from server with imbedded dashes
...
begin
  Sort := GetDfltSort();
  CallV('TMG GET APPOINTMENT LIST', [Provider, FirstDt, LastDt]);
  with RPCBrokerV do
  begin
    //if Sort = 'P' then
    ...
    begin
      ...
      APlace := Piece(x, U, 3);
      ATime := FormatFMDateTime('hh:nn  mmm dd, yyyy', MakeFMDateTime(ATime));
      SetPiece(x, U, 3, ATime+piece(x,U,5));
      x := x + U + APlace;
      Results[i] := x;
    end;
...
//kt added following block 11/12/13
procedure ListPtByHx (Dest: Tstrings; Mode : Int64; FirstDt, LastDt: string);
var DUZ, ModeS : string;
begin
  DUZ := IntToStr(User.DUZ);
  ModeS := IntToStr(Mode);
  CallV('TMG CPRS GET AUDIT PER USER',[DUZ, FirstDt, LastDt, ModeS]);
  MixedCaseList(RPCBrokerV.Results);
  Dest.Assign(RPCBrokerV.Results);
  if Dest.Count=0 then exit;
  if piece(Dest.Strings[0],'^',1) = '-1' then begin
    Dest.Strings[0] := '0^Error: '+ piece(Dest.Strings[0],'^',2)
  end else begin
    if Dest.Count>1 then begin
      if ModeS='1' then Dest.Strings[0] := '0^Patients accessed by you, sorted by Name'
      else Dest.Strings[0] := '0^Patients accessed by you, sorted by Date/Time';
      Dest.Insert(1,'0^---------------------');
    end else begin
      Dest.Strings[0] := '0^No patients found';
    end;
  end;
end;

// VWPT ADDITIONS FOR ENHANCED PATIENT LOOKUP   //kt 9/11 ported
// VWPT ADDITIONS FOR ENHANCED PATIENT LOOKUP
procedure ListPtByOther (Dest: Tstrings; const othertext: string;caption:string );//:string; radbutton:Tobject);
...
begin
  ...
  begin
    ...
    Age := IntToStr(CalcAge(MakeFMDateTime(Piece(x, U, 2)),0));            //elh added back in // Age
    Sex := Piece(x, U, 3);                                                 // Sex
  ...
  //kt begin change   10/23/14
  DueResult := TStringList.Create();
  Result.DueColor := GetPatientDueInfo(DueResult,DFN,Result.DueHint,clBtnFace);
  DueResult.Free;
  //kt end change   10/23/14
end;
...
function GetPatientDueInfo(DueInfo: TStringList; DFN: string; var ToolText: string; DefaultColor:TColor = clInfoBK):TColor;
...
begin
  ...
  if AtFPGLoc() then begin
    DueInfo.Clear;
    result := DefaultColor;    //Default color
    tCallV(DueInfo,'TMG CPRS GET PT DUE STATUS',[DFN]);
    if DueInfo.count >1 then begin
      ColorString := piece(DueInfo[1],'^',1);
      ToolText := piece(DueInfo[1],'^',2);
      result := HexToTColor(ColorString);
    end;
  end else begin
    result := DefaultColor;
  end;

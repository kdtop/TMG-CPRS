...
procedure HideFormulas(SL : TStrings; Formulas : TStringList; var FormulaCount : integer);
procedure DeleteHiddenFormulas(SL : TStrings);
procedure HideTxtObjects(SL : TStrings; TxtObjects : TStringList; var TxtObjCount : integer);
procedure DeleteHiddenTxtObjects(SL : TStrings);
...
function ResolveTemplateFieldsMath(Text: string; Entries: TStringList; AutoWrap: boolean; IncludeEmbedded: boolean = FALSE; HTMLGUIMode : boolean = false): string;
...
const
  ...
  //kt FN_SHOW_TEXT = '{%_____%-#';
  FN_SHOW_TEXT = '{[FORMULA]';
  FN_SHOW_TEXT_END = '}';
  //FN_SHOW_TEXT = '';  //kt 3/16
  //FN_SHOW_TEXT_END = '';  //kt 3/16
...
var
  ...
  VEFANameToObjID : TStringList;  //SL.String[i] = Field Name  Integer(SL.Object[i]) = fieldID
...
begin
  Txt := SL.Text;
  if assigned(Formulas) then Formulas.Clear;
  ...
  while (p1>0) do begin
    ...
    inc(FormulaCount);
    if assigned(Formulas) then Formulas.AddObject(FnStr,Pointer(FormulaCount));
    Txt := SubStrA + FN_SHOW_TEXT + IntToStr(FormulaCount) + FN_SHOW_TEXT_END + SubStrB;
    //kt 3/16 -- p1 := PosEx(FN_BEGIN_SIGNATURE,Txt,p1);
    p1 := Pos(FN_BEGIN_SIGNATURE,Txt);
  end;
  SL.Text := Txt;
end;
...
begin
  ...
  while (p1>0) do begin
    ...
    FnStr := AnsiReplaceText(FnStr,#13,'');
    inc(TxtObjCount);
    TxtObjects.AddObject(FnStr,Pointer(TxtObjCount));
    Txt := SubStrA + OBJ_SHOW_TEXT + IntToStr(TxtObjCount) + OBJ_SHOW_TEXT_END + SubStrB;
...
function ResolveTemplateFieldsMath(Text: string;
                                   ...
                                   HTMLGUIMode : boolean = false): string;
var
  ...
  for ExtMode := vefaFN to vefaOBJ do begin
    repeat
      ...
      if(i > 0) then begin
            ...
            for j := 0 to Entries.Count-1 do begin
              ...
              if(assigned(Entry)) then begin
                ...
                if HTMLGUIMode then begin
                  NewTxt := Entry.GetHTMLControlText(CtrlID, FALSE, FoundEntry, ControlBlockDisabled, iField);  //kt added 1/16
        ...
        //VEFA-261 what was purpose? --> FnObjStr := FormatFormula(FnObjStr);
        ...
        if ExtMode = vefaFN then begin
          ...
          if Pos('???', TempStr) > 0 then begin
            Problem := 'Found "???", which typically indicates problem accessing template field.';
          end else begin
            EvalStr := TempStr;
            TempStr := EvalExpression(TempStr,Problem);
            if (Pos('V',FNFlags)>0) then begin
              TempStr := 'Evaluated: ' + EvalStr + #13#10 + TempStr;
            end;
          end;
          if Problem <> '' then begin

...
uses
  ...
  VA508AccessibilityManager, fBase508Form, VA508ImageListLabeler, Buttons, ToolWin,
  TMGHTML2;
type
...
type
  TfrmDCSumm = class(TfrmHSplit)
    ...
    procedure tvSummsDblClick(Sender: TObject);
    procedure memNewSummKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
    ...
    procedure UseUniversalImageClick(Sender: TObject);     //kt 9/11
  ...
  private
    ...
    TMGForceSaveSwitchEdit : boolean;            //kt 5/15
...
constructor TfrmDCSumm.Create(AOwner: TComponent);
//kt Added function 9/11
begin
  inherited Create(AOwner);
  FViewSumm := TStringList.Create;
end;

destructor TfrmDCSumm.Destroy;
//kt Added function 9/11
begin
  FViewSumm.Free;
  FEditDCSumm.Lines.Free;
  inherited Destroy;
end;
...
begin
  ...
  if EditingIndex <> -1 then
    case BOOLCHAR[frmFrame.CCOWContextChanging] of
    '1': begin
             //kt if memNewSumm.GetTextLen > 0 then
             if EditorHasText then  //kt
...
function TfrmDCSumm.EditorHasText : boolean;
//kt added 9/11
begin
  if (vmHTML in FViewMode) then begin
    ...
    Result := (memNewSumm.GetTextLen > 0);
  end;
end;
...
begin
  ...
  try
    ClearEditControls;
    FEditDCSumm.Lines.Free; //(done d/t full wipe-out on line below}  //kt 9/11
    FillChar(FEditDCSumm, SizeOf(FEditDCSumm), 0);  //v15.7
    FEditDCSumm.Lines := TStringList.Create; //Freed in Destructor    //kt 9/11
    ...
    begin
      if ListItemForEdit <> '' then
        ...
        //EDDIE if TMGForcePlainTextEditMode then begin   //kt  added block 12/27/12
          // Set in TfrmCarePlan.InsertText
          Mode := [vmEdit] + [vmHTML_MODE[False]];   //kt 12/27/12
        //EDDIE  TMGForcePlainTextEditMode := False;
        //EDDIE end else begin
        ...
        //EDDIE if txtSubject.Visible then begin                          //kt 9/11
          memNewSumm.SetFocus;                                    //kt 9/11
        //EDDIE end else begin                                            //kt 9/11
    ...
    begin
      ...
      //EDDIE UpdateNoteAuthor(DocInfo);
      TmpBoilerPlate.Free;
    end;
...
procedure TfrmDCSumm.SetEditorFocus;
...
begin
  ...
  begin
    ...
    begin
      ...
      SetEditorFocus;  //kt memNewSumm.SetFocus;  //kt 9/11
...
begin
  ...
  Mode := [vmEdit] + [vmHTML_MODE[IsHTML(FEditDCSumm.Lines) or (vmHTML in FViewMode)]]; //kt 9/11
  if FEditDCSumm.Lines <> nil then SetDisplayToHTMLvsText(Mode,FEditDCSumm.Lines);   //kt was memNewSumm.Lines.Assign(FEditDCSumm.Lines);
  FChanged := False;
  ...
  lstSummsClick(Self);  // will make pnlWrite visible
  if timAutoSave.Interval <> 0 then timAutoSave.Enabled := True;
  SetEditorFocus;    //kt memNewSumm.SetFocus;
end;
...
var
  ...
  EditIsHTML : boolean; //kt
begin
  Saved := False;
  //kt 5/15 if not (vmHTML in FViewMode) then begin                         //kt 9/11
  ...
  if EditIsHTML then begin
    ...
    EmptyNote := (memNewSumm.GetTextLen = 0) or not ContainsVisibleChar(memNewSumm.Text);
  end;
  //kt 5/15 if not (vmHTML in FViewMode) then begin                         //kt 9/11
  //kt 5/15   EmptyNote := (memNewSumm.GetTextLen = 0)                      //kt 9/11
  //kt 5/15                 or (not ContainsVisibleChar(memNewSumm.Text));  //kt 9/11
  //kt 5/15 end else begin                                                  //kt 9/11
  //kt 5/15   EmptyNote := (HtmlEditor.GetTextLen = 0);                     //kt 9/11
  //kt 5/15   if EmptyNote then begin
  //kt 5/15     EmptyNote := not HTMLContainsVisibleItems(HtmlEditor.HTMLText);
  //kt 5/15   end;
  //kt 5/15 end;

  //kt if (memNewSumm.GetTextLen = 0) or (not ContainsVisibleChar(memNewSumm.Text)) then
  //kt begin
  ...
  if FEditDCSumm.Lines = nil then FEditDCSumm.Lines := TStringList.Create; //kt 9/11
  //kt 5/15 if (vmHTML in FViewMode) then begin
  if EditIsHTML then begin                                          //kt 5/15
    SplitHTMLToArray(WrapHTML(HtmlEditor.HTMLText), FEditDCSumm.Lines); //kt 9/11
    InsertSubs(FEditDCSumm.Lines);                       //kt 9/11
  end else begin                                                    //kt 9/11
    FEditDCSumm.Lines.Assign(memNewSumm.Lines);                     //kt 9/11
  end;
  with FEditDCSumm do
    begin
...
var
  x: string;
  Note      : TStrings;     //kt 9/11  Will be pointer to FViewSumm, or FEditNote.Lines
...
begin
  inherited;
  with lstSumms do begin                                                   //kt 9/11
    //kt with lstSumms do if ItemIndex = -1 then Exit
    ...
    if Editing then begin                                                  //kt 9/11
      ...
      if FEditDCSumm.Lines = nil then FEditDCSumm.Lines := TStringList.Create; //kt 9/11
      Note := FEditDCSumm.Lines;                                             //kt 9/11
      mnuViewDetail.Enabled    := False;
      if FEditDCSumm.IDParent <> 0 then
      ...
      //EDDIE UpdateFormForInput;
      Mode := [vmEdit] + [vmHTML_MODE[(FHTMLEditMode=emHTML)]];            //kt 9/11
      SetDisplayToHTMLvsText(Mode,FEditDCSumm.Lines,VIEW_ACTIVATE_ONLY);     //kt 9/11
      ...
      //kt pnlRead.Visible := True;
      //kt pnlWrite.Visible := False;
...
procedure TfrmDCSumm.ProperRepaint(Editing : Boolean);
//kt added 9/11
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        else if DisplayCosignerDialog(IEN) then mnuActIdentifyAddlSignersClick(Self);  //elh  1/30/14
      end; {if OK}
    end; {if ContinueSign}
  end; {if Length(ESCode)}
...
begin
  ...
  //kt 9/11 HTMLViewer.BackgroundColor := ReadOnlyColor;
  ...
  frmDrawers.ReloadNotes := ReloadSumms;          //kt 5/15
  frmDrawers.DocSelRec.TreeView := tvSumms;        //kt 6/15
  frmDrawers.DocSelRec.TreeType := edseDCSumm;     //kt 6/15
  frmDrawers.NewNoteButton := cmdNewSumm;
  frmDrawers.Splitter := splDrawers;
  frmDrawers.DefTempPiece := 1;
  FImageFlag := TBitmap.Create;
  FDocList := TStringList.Create;
  RestoreRegHTMLFontSize;  //kt 9/11
  HtmlEditor.OnLaunchTemplateSearch := frmDrawers.HandleLaunchTemplateQuickSearch;  //kt 6/15
  HtmlEditor.OnLaunchConsole := frmDrawers.HandleLaunchConsole;                     //kt 6/15
end;
...
procedure TfrmDCSumm.popNoteMemoHTMLFormatClick(Sender: TObject);
begin
  inherited;
  ToggleHTMLEditMode;
end;
...
begin
  ...
  if HTMLEditMode then begin
    ...
    FEditDCSumm.Lines.Text := HTMLEditor.Text;
  end;
...
procedure TfrmDCSumm.tvSummsDblClick(Sender: TObject);
//kt added
begin
  inherited;
  if tvSumms.Selected = nil then exit;
...
procedure TfrmDCSumm.memNewSummKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
procedure TfrmDCSumm.SetDisplayToHTMLvsText(Mode : TViewModeSet;
                                           Lines : TStrings;
...
type
  ...
  procedure SetPanelVisibility(Mode: TPanelVisibilityMode; HTMLMode : boolean);
    ...
    begin
      ...
      if Visible then begin
        memSumm.Visible := not HTMLMode;
        memSumm.TabStop := not HTMLMode;
        pnlHTMLViewer.Visible := HTMLMode;
        HTMLViewer.Visible := HTMLMode;
        if HTMLMode then begin
          HtmlViewer.BringToFront;
        end else begin
          MemSumm.BringToFront;
        end;
      end else begin
        memSumm.Visible := false;
        memSumm.TabStop := false;
    ...
    begin
      ...
      if Visible then begin
        ...
        MemNewSumm.Visible := not HTMLMode;
        if HTMLMode then begin
          HTMLEditor.BringToFront;
        end else begin
          MemNewSumm.BringToFront;
        end;
      end else begin
  ...
  procedure SetHTMLorTextEditor(HTMLEditMode : boolean;
    ...
    begin
      if ActivateOnly=false then begin
        QuickCopyWith508Msg(Lines, memNewSumm); //kt 9/11
        //kt 9/11memNewNote.Lines.Assign(Lines); //kt new
      end;
  ...
  //Set forms such that either HTML Viewer is visible, or standard edit window.
    ...
    begin
      with frmDCSumm do begin
        ...
        if ActivateOnly=False then memSumm.Lines.Assign(Lines);   //new
        memSumm.Visible := true;
        memSumm.TabStop := true;
        memSumm.BringToFront;
        RedrawActivate(memSumm.Handle);
      end;
...
procedure TfrmDCSumm.btnBoldClick(Sender: TObject);
//kt 9/11 added function
begin
  inherited;
...
procedure TfrmDCSumm.btnItalicClick(Sender: TObject);
//kt 9/11 added function
begin
  inherited;
...
procedure TfrmDCSumm.btnLessIndentClick(Sender: TObject);
//kt 9/11 added function
begin
  inherited;
  HtmlEditor.Outdent;
...
procedure TfrmDCSumm.btnMoreIndentClick(Sender: TObject);
//kt 9/11 added function
begin
  inherited;
  HtmlEditor.Indent;
...
procedure TfrmDCSumm.btnRightAlignClick(Sender: TObject);
//kt 9/11 added function
begin
  inherited;
...
function TfrmDCSumm.HTMLResize(ImageFName: string) : string;
var
  NewHeight : real;
  Image: TImage;
const
...
procedure TfrmDCSumm.btnCenterAlignClick(Sender: TObject);
//kt 9/11 added function
begin
  inherited;
...
procedure TfrmDCSumm.btnLeftAlignClick(Sender: TObject);
//kt 9/11 added function
begin
  inherited;
...
procedure TfrmDCSumm.btnNumbersClick(Sender: TObject);
//kt 9/11 added function
begin
  inherited;
...
procedure TfrmDCSumm.btnBulletsClick(Sender: TObject);
//kt 9/11 added function
begin
  inherited;
...
procedure TfrmDCSumm.btnUnderlineClick(Sender: TObject);
//kt 9/11 added function
begin
  inherited;
...
procedure TfrmDCSumm.btnTextColorClick(Sender: TObject);
//kt 9/11 added function
begin
  inherited;
...
procedure TfrmDCSumm.btnBackColorClick(Sender: TObject);
//kt 9/11 added function
begin
  inherited;
...
procedure TfrmDCSumm.btnFontsClick(Sender: TObject);
//kt 9/11 added function
begin
  inherited;
...
procedure TfrmDCSumm.cbFontSizeChange(Sender: TObject);
//kt 9/11 added function
const
...
procedure TfrmDCSumm.cbFontNamesChange(Sender: TObject);
//kt 9/11 added function
var i :  integer;
    FontName : string;
const
...
function GetNodeByIEN(ATree : TORTreeView; IEN:String): TORTreeNode;
var
    Node: TORTreeNode;
begin
  Result := nil;
  if ATree.Items.Count = 0 then Exit;
  Node := TORTreeNode(ATree.Items[0]);
  while Node <> nil do
  begin
    if Piece(Node.StringData,'^',1) = UpperCase(IEN) then
    begin
      Result := Node;
      Break;
    end;
...
procedure TfrmDCSumm.ChangeToNote(IEN : String; ADFN : String = '-1');
//kt 5/15 added
//Effect changing note by simulating a notification click.
var Node: TORTreeNode;
begin
  Node := GetNodeByIEN(tvSumms,IEN);
  if Node <> nil then tvSumms.Selected := Node;
end;

procedure TfrmDCSumm.ReloadSumms;
...
begin
  ...
  LoadSumms;  //This should reload TV etc.
  //ChangeToNote(CurrentNoteIENS);
  Node := GetNodeByIEN(tvSumms,CurrentNoteIENS);
  if Node <> nil then begin
    tvSumms.Selected := Node;
    tvSummsDblClick(self);
  end;
end;


function TfrmDCSumm.ActiveEditIEN : Int64; //kt added entire function
begin
  if EditingIndex >= 0 then Result := lstSumms.GetIEN(EditingIndex)
  else Result := 0;
end;

procedure TfrmDCSumm.InsertNamedImage(FName: string);
//kt 9/11 added function
var
   ImageFName : string;
begin
...
begin
  ...
  //frmImageUpload.CallingForm := 'frmDCSumm';
...
procedure TfrmDCSumm.UseUniversalImageClick(Sender: TObject);
//kt 9/11 added function
//var AddResult: integer;
    //frmImagesMultiUse: TfrmImagesMultiUse;
begin
  inherited;
  {  //incomplete  Originally called from frmNaotes.popupAddImage
...
function TfrmDCSumm.GetNamedTemplateImageHTML(Name: string): string;
//kt 9/11 added function
var Result2: boolean;
    frmImagesMultiUse: TfrmImagesMultiUse;
begin

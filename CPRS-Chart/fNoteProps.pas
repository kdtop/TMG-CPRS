...
function GetNoteTypeOnly: string;  //kt added entire function   8/8/11
        ...
        begin
          case FCallingTab of
           ...
           CT_NOTES:      begin
                            ...
                            cboNewTitle.InitLongList('');
                            //kt  cboAuthor.InitLongList(ANote.AuthorName);
                            //kt  if ANote.Author > 0 then cboAuthor.SelectByIEN(ANote.Author);
                            cboAuthor.InitLongList(User.Name);  //kt  12/27/12
                            if User.DUZ > 0 then cboAuthor.SelectByIEN(User.DUZ);  //kt  12/27/12
...
procedure TfrmNoteProperties.lstRequestsChange(Sender: TObject);  //kt added entire function
begin
...
begin
  frmNoteProperties := TfrmNoteProperties.Create(Application);
  frmNoteProperties.FIsNewNote := True;
  uConsultsList := TStringList.Create;
  mrResult := false;
  try
    ResizeAnchoredFormToFont(frmNoteProperties);
    with frmNoteProperties do begin
      // setup common fields (title, reference date, author)
      FToday := FloatToStr(FMToday);
      FCallingTab := CT_NOTES;
      FIDNoteTitlesOnly := False;
      FClassName := '';
      FIsClinProcNote := ('' = DCL_CLINPROC);
      FCPStatusFlag := 0;
      uShowUnresolvedOnly := True;                         //v26.5 (RV)
      Caption := 'Progress Note Properties';
      GetUnresolvedConsultsInfo; // v26.5 (RV) removed nag screen
      cboNewTitle.InitLongList('');
      cboAuthor.InitLongList(User.Name);
      if User.DUZ > 0 then cboAuthor.SelectByIEN(User.DUZ);
      ListNoteTitlesShort(cboNewTitle.Items);
      cboNewTitle.Enabled := true;
      ShowClinProcFields(FIsClinProcNote);
      FStarting := True;
      cboNewTitle.SelectByIEN(DfltNoteTitle);
      FStarting := False;
      calNote.FMDateTime := FMNow;
      FDocType          := TYP_PROGRESS_NOTE;
      SetCosignerRequired(True);
      SetGenericFormSize;
      cboNewTitle.Caption := lblNewTitle.Caption;
      FStarting := True;
      cboNewTitleExit(frmNoteProperties);        // force display of request/case list
      FStarting := False;
      if uShowUnresolvedOnly then begin              // override previous display if SHOW ME clicked on entrance
        uShowUnresolvedOnly := not uShowUnresolvedOnly;
        FormatRequestList;
      end ;
      lblDateTime.Visible := false;
      calNote.visible := false;
      lblAuthor.Visible := false;
      cboAuthor.visible := false;
      mrResult := ShowModal = idOK;
    end;
  finally
    if mrResult then  Result := frmNoteProperties.cboNewTitle.ItemID
    else Result := '-1';
  end;
  frmNoteProperties.release;

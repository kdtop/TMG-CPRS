...
type
  TfrmSignOrders = class(TfrmBase508Form)
    ...
    procedure fraCoPaylblHNCMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
    ...
    procedure clstCSOrdersMeasureItem(Control: TWinControl; Index: Integer; var AHeight: Integer);
    procedure clstCSOrdersDrawItem(Control: TWinControl; Index: Integer; Rect: TRect; State: TOwnerDrawState);
  ...
  private
    ...
    procedure SetPWSavedVisibility;  //kt added 11/8/15
  public
  ...
  if BILLING_AWARE then begin
    try
      for i := 0 to itemsList.Count-1 do begin
        ...
        if thisOrderID = sourceOrderID then begin   //compare to order ID of source order
...
begin
  ...
  try
    ...
    with SelectedList do for i := 0 to Count - 1 do begin
      obj := TOrder(Items[i]);
      if ((obj.IsControlledSubstance=false) or IsPendingHold(Obj.ID)) then begin
    ...
    with CSSelectedList do for i := 0 to Count - 1 do begin
      ...
      if (obj.IsControlledSubstance and obj.IsOrderPendDC) then begin
        ...
        if (TOrder(Items[i]).DGroupName) = NonVAMedGroup then
          frmSignOrders.clstOrders.State[cidx] := cbGrayed ;
      end else if (obj.IsControlledSubstance and not(IsPendingHold(Obj.ID))) then begin
    ...
    if frmSignOrders.clstCSOrders.Count>0 then begin
      if not(GetPKISite) then begin
        ShowMsg('Digital Signing of Controlled Substances is currently disabled for your site.');
        if frmSignOrders.clstOrders.Count>0 then begin
          frmSignOrders.clstCSOrders.Clear;
          SigItemsCS.ResetOrders;
        end else
          Exit;
      end;
      if not(GetPKIUse ) then begin
    ...
    if SignNotRequired then begin
      frmSignOrders.lblESCode.Visible := False;
      frmSignOrders.txtESCode.Visible := False;
      frmSignOrders.SetPWSavedVisibility; //kt added 11/15
    end;
    if BILLING_AWARE then begin
    ...
    with frmSignOrders do begin
      ...
      if clstCSOrders.Count = 0 then begin
        oheight := pnlOrderList.height;
        pnlProvInfo.Visible := False;
        if fraCoPay.Visible = FALSE then begin
        ...
        Height := newheight;
      end else if clstOrders.Count = 0 then begin
      ...
      end else if fraCoPay.Visible = FALSE then begin
    ...
    if frmSignOrders.AnyItemsAreChecked then begin
    ...
    if not frmSignOrders.lblDEAText.Visible then frmSignOrders.txtESCode.Text := SavedSignature.Value; //kt added 11/15 -- returns '' unless saved in past few minutes.
    frmSignOrders.SetPWSavedVisibility; //kt added 11/15
    frmSignOrders.imgPWSaved.Visible := (Trim(frmSignOrders.txtESCode.Text) <> '');  //kt added 11/15
    if frmSignOrders.txtESCode.Visible then frmSignOrders.ActiveControl := frmSignOrders.txtESCode;

    frmSignOrders.ShowModal;
    if frmSignOrders.OKPressed then begin
      PINResult := prOK;
      Result := True;
      ...
      try
        with SelectedList do for i := 0 to Count - 1 do with TOrder(Items[i]) do begin
          ...
          if (cidx > -1) and (CSOrder) and (cErr = '') then begin
            if frmSignOrders.clstCSOrders.Checked[cidx] then begin
              if IsOrderPendDC then begin
                UpdateOrderDGIfNeeded(ID);
        ...
        if CSSignList.Count > 0 then begin
          LastPINvalue := '';
          ...
          if PINResult = PRError then
            ...
            try
              ...
              for i := 0 to CSSignList.Count - 1 do begin
                ...
                try
                  ...
                  if crypto.Signdata = true then begin
        ...
        if SignList.Count > 0 then begin
          //hds7591  Clinic/Ward movement.  Patient Admission IMO
          if not frmFrame.TimedOut then begin
            if (Patient.Inpatient = True) and (Encounter.Location <> Patient.Location) then begin
              ...
              if OrderPrintList.Count > 0 then begin
              ...
              end else if (clinicList.count = 0) and (wardList.Count = 0) then
                DoNotPrint := True;
        ...
        with SignList do if Count > 0 then for i := 0 to Count - 1 do begin
          if Pos('E', Piece(SignList[i], U, 2)) > 0 then begin
        ...
        StatusText('');
        for cnt := SignList.Count - 1 downto 0 do begin
          if Pos('E', Piece(SignList[cnt], U, 2)) > 0 then begin
            SignList.Delete(cnt);
            Continue;
          end;
...
begin
  ...
  {Begin BillingAware}
  if  BILLING_AWARE then begin
    if SigItems.OK2SaveSettings then begin
      if Not UBACore.BADxEntered then begin   //  if Dx have been entered and OK is pressed
        // billing data will be saved. otherwise error message!
  ...
  if  BILLING_AWARE then begin
    with clstOrders do begin
      if Index < Items.Count then begin
        ...
        if  BILLING_AWARE then begin
          if Assigned(UBAGlobals.tempDxList) then begin
            tempID := TOrder(clstOrders.Items.Objects[Index]).ID;

            if UBAGlobals.tempDxNodeExists(tempID) then begin
              ...
              if Not UBACore.IsOrderBillable(tempID) then begin
...
procedure TfrmSignOrders.clstOrdersClickCheck(Sender: TObject);
  ...
  var
    idx: integer;
  begin
    for idx := 0 to clstOrders.Items.Count - 1 do begin
      if TOrder(clstOrders.Items.Objects[idx]).ParentID = ParentOrderId then begin
        if clstOrders.Checked[idx] <> CheckedStatus then begin
...
begin
  with clstOrders do begin
    if Length(TOrder(Items.Objects[ItemIndex]).ParentID)>0 then begin
  ...
  SetPWSavedVisibility; //kt added 11/15
...
begin
  Result := False;
  with clstCSOrders do begin
    for i := 0 to Items.Count - 1 do begin
      if Checked[i] then begin
        if not TOrder(clstCSOrders.Items.Objects[i]).IsOrderPendDC then begin
          Result := True;
          break;
        end;
      end;
...
begin
  ...
  if (Itm >= 0) then begin
    if (Itm <> FLastHintItem) then begin
     ...
     {Begin BillingAware}
      if  BILLING_AWARE then begin
        ...
        if UBAGlobals.tempDxNodeExists(thisOrderID) then begin
          if Assigned(tempDxList) then
          try
            for i := 0 to (tempDxList.Count - 1) do begin
              thisRec := TBADxRecord(tempDxList.Items[i]);

              if Assigned(thisRec) then begin
                if (thisRec.FOrderID = thisOrderID) then begin
                  with thisRec do begin
...
//      clstOrders.Height := lblESCode.Top - clstOrders.Top - 4;
  ...
  //CQ5172
  if clstOrders.Count = 1 then begin
    ...
    if NOT UBACore.IsOrderBillable(TChangeItem(fOrdersSign.frmSignOrders.clstOrders.Items.Objects[0]).ID) then begin
      buOrdersDiagnosis.Enabled := False;
      Diagnosis1.Enabled := False;
      clstOrders.Selected[0] := False;
    end else if Piece(TChangeItem(fOrdersSign.frmSignOrders.clstOrders.Items.Objects[0]).ID,';',2) = DISCONTINUED_ORDER then begin
      buOrdersDiagnosis.Enabled := False;
      Diagnosis1.Enabled := False;
    end;
  end;
...
begin
  ...
  try
    ...
    for i := 0 to fOrdersSign.frmSignOrders.clstOrders.Items.Count-1 do begin
      if (fOrdersSign.frmSignOrders.clstOrders.Selected[i]) then begin
        ...
        if (tempDxNodeExists(thisOrderID)) then begin
          // Create UBAGlobals.globalDxRec with loaded fields
          if not Assigned(UBAGlobals.globalDxRec) then begin
  ...
  if numSelected = 0 then begin
    ShowMsg(UBAMessages.BA_NO_ORDERS_SELECTED);
    Exit;
  end else if numSelected = 1 then
    match := true;
    ...
    if BILLING_AWARE then begin
      ...
      for i := 0 to fOrdersSign.frmSignOrders.clstOrders.Items.Count-1 do begin
        if (fOrdersSign.frmSignOrders.clstOrders.Selected[i]) then begin
          ...
          if (thisChangeItem = nil) then begin //or (thisChangeItem.ItemType <> CH_ORD)) then
...
begin
  if BILLING_AWARE then begin
    ...
    try
      for i := 0 to clstOrders.Count - 1 do begin
        if (fOrdersSign.frmSignOrders.clstOrders.Selected[i]) then begin
          ...
          if fOrdersSign.targetOrderID = fOrdersSign.srcOrderID then //disallow copying an order to itself
            ...
            //***************************************************************
            if Not UBACore.IsOrderBillable(targetOrderID) then begin
...
begin
  ...
  if BILLING_AWARE then begin
    try
      if Button = mbRight then begin //Right-click to open copy/paste popup menu
        //CQ3325
        if fOrdersSign.frmSignOrders.clstOrders.Items.Count = 1 then begin
          Copy1.Enabled := false;
          Paste1.Enabled := false
        end else begin
          Copy1.Enabled := true;
...
begin
  ...
  if  BILLING_AWARE then begin
    ...
    for i := 0 to clstCSOrders.Items.Count - 1 do begin
      if clstCSOrders.Selected[i] then begin
        thisChangeItem := TChangeItem(clstCSOrders.Items.Objects[i]);

        //Disallow copying of a grid HEADER item on LEFT MOUSE CLICK
        if thisChangeItem = nil then begin
          Copy1.Enabled := false;
          buOrdersDiagnosis.Enabled := false;
          Exit;
        end;

        if (thisChangeItem <> nil) then begin //Blank row - not an order item
          thisOrderList.Clear;
          thisOrderList.Add(thisChangeItem.ID);

          if IsAllOrdersNA(thisOrderList) then begin
            Diagnosis1.Enabled := false;
            buOrdersDiagnosis.Enabled := false;
          end else begin
            Diagnosis1.Enabled := true;
...
procedure TfrmSignOrders.clstCSOrdersClickCheck(Sender: TObject);
   ...
   var
     idx: integer;
   begin
     for idx := 0 to clstCSOrders.Items.Count - 1 do begin
       if TOrder(clstCSOrders.Items.Objects[idx]).ParentID = ParentOrderId then begin
         if clstCSOrders.Checked[idx] <> CheckedStatus then begin
...
begin
  ...
  if CSItemsAreChecked and nonDCCSItemsAreChecked then begin
    lblDeaText.Visible := TRUE;
  ...
  if AnyItemsAreChecked then begin
    lblESCode.Visible := IsSignatureRequired;
    txtESCode.Visible := IsSignatureRequired
  end else begin
    lblESCode.Visible := ItemsAreChecked or CSItemsAreChecked;
...
begin
  inherited;
  AHeight := SigItemHeight;
  with clstCSOrders do if Index < Items.Count then begin
    ARect := ItemRect(Index);
    Canvas.FillRect(ARect);
    x := FilteredString(Items[Index]);
    AHeight := WrappedTextHeightByFont(Canvas, Font, x, ARect);
...
begin
  ...
  if  BILLING_AWARE then begin
    ...
    for i := 0 to clstOrders.Items.Count - 1 do begin
      if clstOrders.Selected[i] then begin
        thisChangeItem := TChangeItem(clstOrders.Items.Objects[i]);
...
begin
  inherited;
  if FOSTFHintWndActive then begin
  ...
  case Key of
     ...
     //CQ5054
     83,115: if (ssAlt in Shift) then begin
...
begin
  ...
  if fOrdersSign.frmSignOrders.clstCSOrders.count = 0 then begin
  ...
  if fOrdersSign.frmSignOrders.clstOrders.count = 0 then begin
...
begin
  ...
  for i := 0 to fOrdersSign.frmSignOrders.clstOrders.Count-1 do begin
    if fOrdersSign.frmSignOrders.clstOrders.Items.Objects[i] <> nil then begin //Not a Group Title
      if fOrdersSign.frmSignOrders.clstOrders.Items.Objects[i] is TOrder then begin
...
procedure TfrmSignOrders.SetPWSavedVisibility;
//kt added 11/8/15
begin
    //lblSmartCardNeeded
end;

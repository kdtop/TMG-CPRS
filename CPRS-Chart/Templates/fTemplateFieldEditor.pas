...
type
  TfrmTemplateFieldEditor = class(TfrmBase508Form)
    ...
    lblMin: TLabel;
    udInc: TUpDown;  //kt mod. On form, changed udInc.Min from 1 --> 0
    edtInc: TCaptionEdit;
    lblInc: TLabel;
    lblMaxVal: TLabel;
    ...
    cboDataMode: TComboBox;
    lblDBMode: TLabel;
    procedure cboDataModeChange(Sender: TObject);  //kt added 5/16
...
var
  ...
  Y, idx, Max: integer;
  wpTxt: string;
  TempText, DateLblText : string; //kt 5/16
  ...
  if(not FUpdating) then begin
    ...
    try
      ...
      if(ok) then begin
        ...
        //kt original --> txt := (FFld.FldType in EditDfltTypes);
        txt := (FFld.FldType in EditDfltTypes) and (FFld.DataBinding = dbcbNone); //kt 5/16
      end else begin
        edt := FALSE;
        txt := FALSE;
      end;
      ...
      //kt TEMP!! replace later -->   pnlSwap.DisableAlign;
      pnlSwap.DisableAlign;  //1/24/17  added back from previously being commented out because fields were not widening like they should
      try
        ...
        //kt begin mod 6/16 ---------------
        cboDataMode.Enabled := ok;
          //cbDataControl.Checked := ok and FFld.IsDBControl;
        if ok then begin
          cboDataMode.ItemIndex := ord(FFld.DataBinding);
        end else begin
          cboDataMode.ItemIndex := -1;
          cboDataMode.Text := '';
        end;
        //kt end mod 6/16 ---------------

        lblLength.Enabled := edt;
        if ok and (FFld.FldType = dftWP) then begin
        ...
        pnlNum.Visible := tmp;
        TempText := DefTxt;                                              //kt added 6/16
        if ok and FFld.IsDBControl_Reader then TempText := '';           //kt added 6/16
        SetY(pnlNum, TempText);  //kt 6/16
        //kt original --> SetY(pnlNum, DefTxt);                          //kt 6/16
        if tmp then begin
          udDefNum.visible := not FFld.IsDBControl_Reader;               //kt added 6/16
          edtDefNum.visible := not FFld.IsDBControl_Reader;              //kt added 6/16
          if FFld.IsDBControl then edtDefault.Visible := false;          //kt added 6/16
          ...
          FFld.Increment := udInc.Position;                              //kt added
        end;

        tmp := ok and (FFld.FldType = dftDate);
        pnlDate.Visible := tmp;
        //kt original --> SetY(pnlDate, DefTxt);
        DateLblText := DefTxt;                                           //kt 5/16
        if ok and FFld.IsDBControl_Reader then DateLblText := '';        //kt 7/16
        SetY(pnlDate, DateLblText);                                      //kt 7/16
        if tmp then begin
        ...
        tmp := ok and (FFld.FldType in ItemDfltTypes) and not FFld.IsDBControl_Reader;  //kt 7/16
        ...
        if tmp and (FFld.FldType in EditTextTypes) and FFld.IsDBControl_Reader then tmp := false; //kt 7/16
        reItems.Visible := tmp;
        lblLine.Visible := tmp;
        lblCol.Visible := tmp;
        if tmp then begin
...
begin
  if DoUpdate and FUpdating then begin
    exit;
  end;
  Max1 := 0;
  Max2 := 0;
  ...
  try
    ...
    idx := -1;
    if(assigned(FFld)) and reItems.Visible and cbxDefault.Visible then begin
      ChangeSizes := TRUE;
      for i := 0 to reItems.Lines.Count-1 do begin
        Siz := length(StripEmbedded(reItems.Lines[i]));
        if Max1 < Siz then begin
          Max1 := Siz;
        end;
        if(idx < 0) and (FFld.ItemDefault = reItems.Lines[i]) then begin
          idx := i;
        end;
      end;
      Max2 := Max1;
      if Max1 > MaxTFEdtLen then begin
        Max1 := MaxTFEdtLen;
      end;
    end;
  ...
  if ChangeSizes and DoUpdate then begin
    udLen.Position := Max1;
    if (not udTextLen.Enabled) or
       ((udTextLen.Position > 0) and (udTextLen.Position < Max2)) then begin
...
begin
  ...
  //kt begin mod 6/16 -------------
  cboDataMode.Items.Clear;
  for i := 0 to ord(dbcbWriteOnly) do begin
    cboDataMode.Items.Add(DB_CONTROL_BINDING_LABELS[TDBControlBinding(i)]);
  end;
  //kt end mod 6/16 -------------
  UpdateControls;
  ...
  pnlSwap.Height := lblLM.Top - pnlSwap.Top - 8; //kt added "-8" 7/16
...
begin
  if(cbxObjs.ItemIEN <> 0) then begin
...
procedure TfrmTemplateFieldEditor.cboDataModeChange(Sender: TObject);
//kt added 6/16
begin
  inherited;
  if (not FUpdating) and assigned(FFld) then begin
    FFld.DataBinding := TDBControlBinding(cboDataMode.ItemIndex);
    cboDataMode.ItemIndex := Ord(FFld.DataBinding);
    FChangesPending := TRUE;
    UpdateControls;
  end;
...
begin
  ...
  if assigned(frmTemplateFields) then begin
    if ((Sender = reItems) or (Sender = edtDefault)) then begin
      frmTemplateFields.btnInsert.Enabled := TRUE;
      if Sender = reItems then begin
...
begin
  ...
  if iCon <> nil then begin
    if iCon.Name <> ActiveControl.Name then begin
      ActiveControl := iCon;
      iCon.SelStart := iCon.SelLength;
    end;
    if (not assigned(frmTemplateFields)) then begin
      frmTemplateFields := TfrmTemplateFields.Create(Self);
      frmTemplateFields.Font := Font;
    end;
    ShowButton := False;
    if iCon = reItems then begin
      frmTemplateFields.re := reItems;
      ShowButton := TRUE;
    end else if iCon = edtDefault then begin
      frmTemplateFields.re := edtDefault;
      ShowButton := TRUE;
    end;

...
type
  ...
  private
    ...
    FViewNote : TStringList;   //kt
    
    //FMousing: TDateTime;
    procedure BroadcastImages(Note: TStrings);  //kt  5/6/14
  ...
  public
    ...
    //HtmlResultViewer : THtmlObj; //kt 9/11
    HtmlResultViewer : TWebBrowser; //kt 9/11
    procedure SetDisplayToHTMLvsText(Mode :TViewModeSet; Lines : TStrings; ActivateOnly : boolean=False); //kt 9/11
    procedure UpdateList;  //kt moved from private  4/30/14 
...
constructor TfrmConsults.Create(AOwner: TComponent);
...
destructor TfrmConsults.Destroy;
//kt Added function 9/11
begin
  FViewNote.Free;
...
begin
  ...
  uChanging := False;
  //HTMLResultViewer.Clear;   //kt 9/11
...
//      tvConsults.Enabled := True;
      ...
      pnlRead.BringToFront;
      memConsult.TabStop := True;
      if Copy(Piece(lstNotes.ItemID, ';', 2), 1, 4)= 'MCAR' then begin
        ...
        if not FWarmedUp and IsHTML then begin
          FWarmedUp := True;
          SetDisplayToHTMLvsText(Mode,FViewNote);
        end;
        memConsult.SelStart := 0;
      end;
      if Copy(Piece(lstNotes.ItemID, ';', 2), 1, 4) <> 'MCAR' then begin
      ...
      pnlRight.Refresh;
      if IsHTML then begin  //kt for block
        HTMLResultViewer.Refresh;
        HTMLResultViewer.Repaint;
      end else begin
        memConsult.Repaint;
        memResults.Repaint;
      end;
      Application.ProcessMessages;
      fImages.HTMLEditor := frmConsults.HtmlResultViewer;  //kt
      frmImages.NewNoteSelected(False);                    //kt 9/05
      DisplayPCE;                                          //kt 9/11 (move down from above)
      BroadcastImages(frmConsults.FViewNote);              //kt 9/11}
    end;
    SetResultMenus;
    StatusText('');
  end;
...
procedure TfrmConsults.BroadcastImages(Note: TStrings);
//kt added 9/11
var
...
begin
  ...
  HtmlResultViewer := TWebBrowser.Create(pnlHTMLResultViewer);
  //HtmlResultViewer := THtmlObj.Create(pnlHTMLResultViewer,Application);
  TWinControl(HtmlResultViewer).Parent:=pnlHTMLResultViewer;
  TWinControl(HtmlResultViewer).Align:=alClient;
  ...
  //HTMLResultViewer.PopupMenu := popNoteMemo;
//TWinControl(HtmlEditor).Parent:=pnlHTMLEdit;
  ...
  UpdateReadOnlyColorScheme(HTMLResultViewer, TRUE);      //kt 9/11
  frmDrawers.DocSelRec.TreeView := tvConsults;        //kt 6/15  -- finish implementing later
  frmDrawers.DocSelRec.TreeType := edseConsults;      //kt 6/15  -- finish implementing later
...
function TfrmConsults.ActiveEditIEN : Int64; //kt added entire function
begin
  if EditingIndex >= 0 then Result := lstConsults.GetIEN(EditingIndex)
  else Result := 0;
end;
...
procedure TfrmConsults.SetDisplayToHTMLvsText(Mode : TViewModeSet;
                                           Lines : TStrings;
...
type
  ...
  procedure SetPanelVisibility(Mode: TPanelVisibilityMode; HTMLMode : boolean);
    ...
    begin
      ...
      if Visible then begin
        memConsult.Visible := not HTMLMode;
        memConsult.TabStop := not HTMLMode;
        pnlHTMLResultViewer.Visible := HTMLMode;
        HTMLResultViewer.Visible := HTMLMode;
        if HTMLMode then begin
          HTMLResultViewer.BringToFront;
        end else begin
          MemConsult.BringToFront;
        end;
      end else begin
        memConsult.Visible := false;
        memConsult.TabStop := false;
        pnlHTMLResultViewer.Visible := false;
        HTMLResultViewer.Visible := false;
      end;
    ...
    begin
      ...
      if Visible then begin
        ...
        MemNewConsult.Visible := not HTMLMode;
        if HTMLMode then begin
          HTMLEditor.BringToFront;
        end else begin
          MemNewConsult.BringToFront;
end;
      end else begin
  ...
  procedure SetHTMLorTextEditor(HTMLEditMode : boolean;
    ...
    begin
      if ActivateOnly=false then begin
        QuickCopyWith508Msg(Lines, memNewConsult); //kt 9/11
        //kt 9/11memNewConsult.Lines.Assign(Lines); //kt new
      end;
  ...
  //Set forms such that either HTML Viewer is visible, or standard edit window.
    ...
    procedure ActivateHTMLResultViewer(Lines : TStrings);
    begin
      with frmConsults do begin

        pnlHTMLResultViewer.Visible := true;
        //memConsult.Visible := false;
        //memConsult.TabStop := false;
        HTMLResultViewer.BringToFront;
        if ActivateOnly=False then begin
          FixHTML(Lines);
          //HTMLResultViewer.HTMLText := Lines.Text;
          WBLoadHTML(HTMLResultViewer,Lines.Text);
        end;
        //HTMLResultViewer.Editable := false;
        //HTMLResultViewer.BackgroundColor := clCream;
        HTMLResultViewer.TabStop := true;
        RedrawActivate(HTMLResultViewer.Handle);
      end;
    end; {SetDisplayToHTMLvsText.SetHTMLorTextViewer.ActivateHTMLResultViewer}

    procedure ActivateMemoViewer(Lines : TStrings);
    begin
      with frmConsults do begin
        pnlHTMLResultViewer.Visible := false;
        //HTMLResultViewer.Clear;
        HTMLResultViewer.Navigate('about:blank');
        //HtmlEditor.Clear;  //don't this here.  Only clear in ClearEditControls...
        HTMLResultViewer.TabStop := false;
        if ActivateOnly=False then memConsult.Lines.Assign(Lines);   //new
        memConsult.Visible := true;
        memConsult.TabStop := true;
        memConsult.BringToFront;
        RedrawActivate(memConsult.Handle);
      end;
  ...
  begin {SetHTMLorTextViewer}
    ...
    if HTMLViewMode then begin
      ActivateHTMLResultViewer(Lines);
    end else begin

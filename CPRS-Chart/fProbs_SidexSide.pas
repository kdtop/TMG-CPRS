...
begin
  ...
  if frmFrame.TabCtrlClicked and (frmFrame.ChangingTab[frmFrame.TabCtrlClickedSide] = CT_PROBLEMS) then frmFrame.ProbTabClicked := True;
  if (bbNewProb.CanFocus) and (not pnlButtons.Visible) and ((not PTSwitchRefresh) or frmFrame.ProbTabClicked) then bbNewProb.SetFocus;
  if PTSwitchRefresh then PTSwitchRefresh := False;
  if frmFrame.TabCtrlClicked then frmFrame.TabCtrlClicked := False;
  if frmFrame.ProbTabClicked then frmFrame.ProbTabClicked := False;
  finally
    tempComp.Free;
  end;
end;

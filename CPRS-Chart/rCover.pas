...
begin
  ...
  begin
    ...
    if AtFPGLoc then MarkForDelete := False;     //TMG added to allow all server returned meds   1/26/17
...
begin
  RPCResult := sCallV('TMG DOES HTML FACESHEET EXIST', []);
  result := (strtoint(RPCResult) > 0);
end;

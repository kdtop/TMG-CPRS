...
uses
  ...
  OleServer, Variants, Word2000, ArrowCha, ORDtTm, uGraphs, fBase508Form, rWVEHR
  {$IFDEF VER140}
  ,Word97;
  {$ELSE}
...
type
  TfrmGraphs = class(TfrmBase508Form)
    ...
    procedure lvwItemsBottomChange(Sender: TObject; Item: TListItem; Change: TItemChange);
    procedure lvwItemsBottomClick(Sender: TObject);
    procedure lvwItemsBottomColumnClick(Sender: TObject; Column: TListColumn);
    procedure lvwItemsBottomCompare(Sender: TObject; Item1, Item2: TListItem; Data: Integer; var Compare: Integer);
    procedure lvwItemsTopChange(Sender: TObject; Item: TListItem; Change: TItemChange);
    procedure lvwItemsTopClick(Sender: TObject);
    procedure lvwItemsTopColumnClick(Sender: TObject; Column: TListColumn);
    procedure lvwItemsTopCompare(Sender: TObject; Item1, Item2: TListItem; Data: Integer; var Compare: Integer);
    procedure lvwItemsTopKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);

    procedure cboDateRangeChange(Sender: TObject);
    procedure cboDateRangeDropDown(Sender: TObject);

    procedure chartBaseClickLegend(Sender: TCustomChart; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
    procedure chartBaseClickSeries(Sender: TCustomChart; Series: TChartSeries; ValueIndex: Integer; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
    procedure chartBaseMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
    procedure chartBaseMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
    procedure chartBaseMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
    procedure serDatelineTopGetMarkText(Sender: TChartSeries; ValueIndex: Integer; var MarkText: String);
  ...
  private
    ...
    FSortCol: integer;
    FSearchTerms : TStringList; //kt added 6/16
    SearchDelayTimer : TTimer; //kt added 6/16
...
begin
  ...
  FSearchTerms := TStringList.Create; //kt added 6/16
  SearchDelayTimer := TTimer.Create(Self); //kt added 6/16
  SearchDelayTimer.OnTimer := HandleSearchDelayTimer; //kt added 6/16
end;
...
begin
  ...
  if pnlMain.Tag > 0 then begin
    rptviews := MixedCase(rpcReportParams(pnlMain.Tag));
    if length(rptviews) > 1 then begin
  ...
  if lstViewsTop.ItemIndex > -1 then begin
    lstViewsTopChange(self)
  end else begin
    lvwItemsTopClick(self);
  end;
  if lstViewsBottom.ItemIndex > -1 then begin
...
begin
  ...
  with lstViewsTop do begin
    if GtslViews.Count > 0 then begin
      if not ((GtslViews.Count = 1) and (Piece(GtslViews[0], '^', 1) = VIEW_CURRENT)) then begin
        Items.Add(LLS_FRONT + copy('Temporary Views' + LLS_BACK, 0, 60) + '^0');
        for i := 0 to GtslViews.Count - 1 do begin
          listline := GtslViews[i];
          if Piece(listline, '^', 1) <> VIEW_CURRENT then begin
            Items.Add(VIEW_TEMPORARY + '^' + listline + '^');
          end;
        end;
      end;
    end;
    if GtslViewPersonal.Count > 0 then begin
      Items.Add(LLS_FRONT + copy('Personal Views' + LLS_BACK, 0, 60) + '^0');
      for i := 0 to GtslViewPersonal.Count - 1 do begin
        Items.Add(VIEW_PERSONAL + '^' + GtslViewPersonal[i] + '^');
      end;
    end;
    if GtslViewPublic.Count > 0 then begin
      Items.Add(LLS_FRONT + copy('Public Views' + LLS_BACK, 0, 60) + '^0');
      for i := 0 to GtslViewPublic.Count - 1 do begin
...
begin
  ...
  for i := 0 to aList.Count - 1 do begin
    ...
    if length(value) > 1 then begin
      ...
      if FGraphType = GRAPH_FLOAT then begin
        if name = 'FBOUNDS' then begin
          if value = '0,0,0,0' then begin
...
//kt added function
//Purpose is to fix a memory leak which would happen when list was just cleared
//   without first freeing the held TGraphItem objects
//NOTE: instantiated TGraphItem's can be added to either lvwItemsTop or lvwItemsBottom
//     And sometimes the bottom is assigned the values of the top.  So we can
//    have pointers to the same TGraphItem in both lists.

  procedure ClearOneListviewPtrs(LvwItems : TListView; ListOfFreed : TList);
  var i, j : integer;
      aListItem: TListItem;
      aGraphItem : TGraphItem;
  begin
    for i := 0 to LvwItems.Items.Count - 1 do begin
      aListItem := LvwItems.Items[i];
      if not assigned(aListItem) then continue;
      if not assigned(aListItem.SubItems) then continue;
      for j := 0 to aListItem.SubItems.Count - 1 do begin
        if not assigned(aListItem.SubItems.Objects[j]) then continue;
        aGraphItem := TGraphItem(aListItem.SubItems.Objects[j]);
        aListItem.SubItems.Objects[j] := nil;
        if ListOfFreed.IndexOf(aGraphItem) > -1 then continue;  //avoid freeing twice.
        ListOfFreed.Add(aGraphItem);
        aGraphItem.Free;
      end;
    end;
  end;

var ListOfFreed : TList; //temporarily holds list of old, bad pointers (already freed)
begin
  ListOfFreed := TList.create;
  ClearOneListviewPtrs(lvwItemsTop, ListOfFreed);
  ClearOneListviewPtrs(lvwItemsBottom, ListOfFreed);
...
begin
  //kt lvwItemsTop.Items.Clear;
  //kt lvwItemsBottom.Items.Clear;
  ClearLvwItems; //kt added 6/16
...
begin
  ...
  //kt lvwItemsTop.Items.Clear;
  //kt lvwItemsBottom.Items.Clear;
  ClearLvwItems; //kt 6/16
  lvwItemsTop.SortType := stNone; // if Sorting during load then potential error
  lvwItemsBottom.SortType := stNone; // if Sorting during load then potential error
  if (cboDateRange.ItemIndex > 0) and (cboDateRange.ItemIndex < 9) then begin
    ...
    for i := 0 to GtslItems.Count - 1 do begin
      ...
      if TypeIsDisplayed(filenum) then begin
        lastdate := strtofloatdef(Piece(GtslItems[i], '^', 6), -BIG_NUMBER);
        if (lastdate > oldestdate) and (lastdate < newestdate) then begin
  ...
  end else if (cboDateRange.ItemIndex = 0) or (cboDateRange.ItemIndex > 8) then begin     // manual date range selection
    for i := 0 to GtslAllTypes.Count - 1 do begin
...
begin
  ...
  oldtempiteminfo := '';
  with lvwItemsTop do
  for i := 0 to GtslScratchTemp.Count - 1 do begin
    tempiteminfo := GtslScratchTemp[i];
    tempitemnum := UpperCase(Piece(tempiteminfo, '^',2));
    for j := 0 to GtslItems.Count - 1 do begin
      iteminfo := GtslItems[j];
      if filenum = UpperCase(Piece(iteminfo, '^', 1)) then begin
        if filenum = '690' then begin  // recheck medicine dates & duplicates
          meddate := strtofloatdef(Piece(iteminfo, '^', 6), -BIG_NUMBER);
          if tempiteminfo <> oldtempiteminfo then begin
            if (meddate <> -BIG_NUMBER) and (meddate > oldestdate) and (meddate < newestdate) then begin
              if tempitemnum = UpperCase(Piece(iteminfo, '^', 2)) then begin
                UpdateView(filename, filenum, tempitemnum, iteminfo, lvwItemsTop);
                oldtempiteminfo := tempiteminfo;
              end;
            end;
          end;
        end else if tempitemnum = UpperCase(Piece(iteminfo, '^', 2)) then begin
          UpdateView(filename, filenum, tempitemnum, iteminfo, lvwItemsTop)
        end else if filenum = '63' then begin
          itemnum := UpperCase(Piece(iteminfo, '^', 2));
          if tempitemnum = Piece(itemnum, '.', 1) then begin
            if DateRangeMultiItems(oldestdate, newestdate, itemnum) then begin
...
//kt doc.  aListView always seems to be lvwItemsTop.  I can' find it called with lvwItemsBottom
//kt added dependence on FSearchTerms : TStringList
var
  drugclass, itemname, itemqualifier: string;
  aGraphItem: TGraphItem;
  aListItem: TListItem;
  i : integer; //kt added 6/16
  ShouldHide : boolean; //kt added 6/16
  UCItemName : string;
begin
  if filenum = '63' then begin
    itemqualifier := Piece(itemnum, '.', 2);
    if length(itemqualifier) > 0 then begin
      if FGraphSetting.MergeLabs then begin
        if itemqualifier  <> '0' then exit;
      end;
      if not FGraphSetting.MergeLabs then begin
        if itemqualifier  = '0' then exit;
      end;
    end;
  end;
  ...
  //kt begin mod 6/16 -------------------
  ShouldHide := false;
  UCItemName := UpperCase(itemname);
  for i := 0 to FSearchTerms.Count - 1 do begin
    if Pos(UpperCase(FSearchTerms.Strings[i]), UCItemName) > 0 then continue;
    ShouldHide := true;
    break;
  end;
  if ShouldHide then exit;
  //kt end mod 6/16 -------------------
  ...
  with aListItem do begin
    ...
    aGraphItem := TGraphItem.Create;   //kt 6/16 I can't find where these are free'd on destruction. Fixed via ClearLvwItems
...
begin
  ...
  if GraphTurboOn then
    Switch;
  //if not FFastItems then
  if GtslItems.Count = 0 then begin
    for i := 0 to GtslTypes.Count - 1 do begin
      dfntype := Piece(GtslTypes[i], '^', 1);
      if TypeIsLoaded(dfntype) then begin
...
procedure TfrmGraphs.HandleSearchDelayTimer(Sender: TObject);
//kt added function 6/16
var lvItem: TListItem;
begin
  SearchDelayTimer.Enabled := false;
  cboDateRangeChange(Sender);  //This will trigger redisplay of labs for currently specified dates.
  lstViewsTop.ClearSelection;
  lvwItemsTop.ClearSelection;
  if edtSearch.Text = '' then exit;
  lvItem := lvwItemsTop.FindCaption(0,edtSearch.Text,True,True,False);
  if lvItem <> nil then begin
    lvwItemsTop.ClearSelection;
    lvwItemsTop.Selected := lvItem;
    lvItem.MakeVisible(True);
    lvwItemsTopClick(Sender);
    //lvItem.MakeVisible(True);
...
procedure TfrmGraphs.edtSearchChange(Sender: TObject);
//kt added function 4/7/15, modified 6/16
begin
  inherited;
  SearchDelayTimer.Enabled := false;
  SearchDelayTimer.Interval := 200;
  SearchDelayTimer.Enabled := true;  //reset timer
  FSearchTerms.Clear;
  PiecesToList(edtSearch.Text, ' ', FSearchTerms);
  {
  if edtSearch.Text = '' then exit;
  lvItem := lvwItemsTop.FindCaption(0,edtSearch.Text,True,True,False);
  if lvItem <> nil then begin
...
begin
  ...
  for i := 1 to 2 do begin
    ...
    while aListItem <> nil do begin
      ...
      if filetype = '63' then begin
        typeitem := UpperCase(Piece(aGraphItem.Values, '^', 2));
        if Piece(typeitem, '.', 2) = '0' then begin
          Result := true;
          exit;
        end;
      end;
...
begin
  FNonNumerics := false;
  if section = 'top' then pnlItemsTop.Tag := 0 else pnlItemsBottom.Tag := 0;
  if aListView.SelCount = 1 then begin          // one lab test - make separate
    aListItem := aListView.Selected;
    aGraphItem := TGraphItem(aListItem.SubItems.Objects[3]);
    filetype := UpperCase(Piece(aGraphItem.Values, '^', 1));
    if (filetype = '63') or (filetype = '120.5') then begin
...
procedure TfrmGraphs.MakeTogether(aScrollBox: TScrollBox;
                                  aListView: TListView;
                                  aPadPanel: TPanel; section: string);
var
...
begin
  ...
  with aChart do begin
    if btnChangeSettings.Tag = 1 then begin
      LeftAxis.Title.Caption := 'StdDev';
    end;
    Visible := true;
  end;
...
begin
  ...
  while aListItem <> nil do begin
    ...
    if graphtype = 1 then begin
      ...
      MakeLineSeries(aChart, aTitle, filetype, section, lcnt, ncnt, true);
      if FDisplayFreeText = true then DisplayFreeText(aChart);
    end else if section = 'top' then begin
      MakeDateline(section, aTitle, filetype, chartDatelineTop, graphtype, bcnt, pcnt, gcnt, vcnt)
    end else begin
      MakeDateline(section, aTitle, filetype, chartDatelineBottom, graphtype, bcnt, pcnt, gcnt, vcnt);
    end;
...
begin
  ...
  if bottomview > 0 then begin
    ...
    while aListItem <> nil do begin
      ...
      for j := 0 to lvwItemsTop.Items.Count - 1 do begin
        aGraphItem := TGraphItem(lvwItemsTop.Items[j].SubItems.Objects[3]);
        if typeitem = Pieces(aGraphItem.Values, '^', 1, 2) then begin
  ...
  if topview > 0 then begin
    lstViewsBottom.ItemIndex := topview;
    lstViewsBottomChange(self);
  end else begin
    lstViewsBottom.ItemIndex := -1;
    lvwItemsBottom.ClearSelection;
    for i := 0 to GtslScratchSwap.Count - 1 do begin
      for j := 0 to lvwItemsBottom.Items.Count - 1 do begin
        aGraphItem := TGraphItem(lvwItemsBottom.Items.Item[j].SubItems.Objects[3]);
        if aGraphItem.Values = GtslScratchSwap[i] then begin
...
procedure TfrmGraphs.mnuPopGraphExportClick(Sender: TObject);
  ...
  begin
    ...
    while aListItem <> nil do begin
      ...
      for i := 0 to GtslData.Count - 1 do begin
        datax := GtslData[i];
        if Piece(datax, '^', 1) = itemtype then begin
          if Piece(datax, '^', 2) = item then begin
            ...
            if DatesInRange(uDateStart, uDateStop, dtdata1, dtdata2) then begin
...
begin
  ...
  ShortHeader := Patient.Name + '         ' + Patient.SSN + '          '
         ...
         + FormatFMDateTime('mmm dd, yyyy', Patient.DOB) + ' (' + GetPatientBriefAge(Patient.DFN) + ')'
...
begin
  ...
  if StripHotKey(mnuPopGraphIsolate.Caption) = ('Move all selections to ' + aOtherSection) then begin
    aListItem := aListView.Selected;
    while aListItem <> nil do begin
      aGraphItem := TGraphItem(aListItem.SubItems.Objects[3]);
      typeitem := Pieces(aGraphItem.Values, '^', 1, 2);
      for j := 0 to aOtherListView.Items.Count - 1 do begin
        aGraphItem := TGraphItem(aOtherListView.Items.Item[j].SubItems.Objects[3]);
        if Pieces(aGraphItem.Values, '^', 1, 2) = typeitem then begin
    ...
    for i := 0 to aOtherListView.Items.Count - 1 do begin
...
begin
  ...
  with aListView do begin
    for i := 0 to Items.Count - 1 do begin
      if Items[i].Caption = aItemName then begin
  ...
  if aNum = -1 then begin
    ...
    with aListView do begin
      for i := 0 to Items.Count - 1 do begin
        if Items[i].Caption = aItemName then begin           // match without (specimen)
          aGraphItem := TGraphItem(Items.Item[i].SubItems.Objects[3]);     //get file^ien match
          aNum := i;
          aTypeItem := Pieces(aGraphItem.Values, '^', 1, 2);
          break;
...
var
  ...
  aListItem: TListItem;
begin
  if chartDatelineTop.Tag = 1 then begin               // series
    ItemCheck(lvwItemsTop, mnuPopGraphIsolate.Hint, selnum, typeitem);
    if selnum < 0 then exit;
    if not HSAbbrev(Piece(typeitem, '^', 1)) then begin
      if (FGraphSeries is TGanttSeries) then begin
...
begin
  ...
  begin
    ...
    tmpItem := Patient.Name + '         ' + Patient.SSN + '          '
             + Encounter.LocationName + '                            '
             //WV patient age change
             + FormatFMDateTime('mmm dd, yyyy', Patient.DOB) + ' (' + GetPatientBriefAge(Patient.DFN) + ')';
//             + FormatFMDateTime('mmm dd, yyyy', Patient.DOB) + ' (' + IntToStr(Patient.Age) + ')';
              //WV end pateint age change
    Add(tmpItem);
    Add(TXT_COPY_DISCLAIMER);
    Add(DateRange);
    Add(Warning);
  ...
  if (cboDateRange.ItemID = 'S') then begin
    with calDateRange do begin
      if Execute then begin
        if Length(TextOfStart) > 0 then begin
          if Length(TextOfStop) > 0 then begin
            dateranges :=
...
begin
  ...
  while aListItem <> nil do begin
    aGraphItem := TGraphItem(aListItem.SubItems.Objects[3]);
    typeitem := UpperCase(aGraphItem.Values);
    if Piece(typeitem, '^', 1) = '63' then begin
      specnum := Piece(Piece(typeitem, '^', 2), '.', 2);
      if length(specnum) > 0 then begin // multispecimen
    ...
    if length(typeitem) > 0 then begin
...
begin    // uses lvwItems... Tag as index for view selection
  ...
  with lvwItemsTop do begin
    if (Tag = 0) and (length(lvwItemsTop.Hint) > 0) then begin
      for i := 0 to lstViewsTop.Items.Count - 1 do begin
        if lvwItemsTop.Hint = Piece(lstViewsTop.Items[i], '^', 2) then begin
          Tag := i;
          break;
        end;
      end;
...
begin
  ...
  begin
    ...
    if Piece(typeitem, '^', 1) = '63' then begin
      ...
      for j := 0 to GtslCheck.Count - 1 do begin
        testcheck := GtslCheck[j];
        if Pieces(testcheck, '^', 1, 2) = typeitem then begin
          if Piece(testcheck, '^', 3) = '*' then begin
            flag := true;
            break;
          end;
        end;
  ...
  if flag then begin
    ...
    while aListItem <> nil do begin
      ...
      typeitem := UpperCase(Pieces(aGraphItem.Values, '^', 1, 2));
      if Piece(typeitem, '^', 1) = '63' then begin
        if length(Piece(typeitem, '.', 2)) = 0 then begin
          for i := 0 to GtslCheck.Count - 1 do begin
              ...
              if Piece(testcheck, '^', 3) = '*' then begin
                if FGraphSetting.MergeLabs then begin
                  ...
                  for j := 0 to GtslCheck.Count - 1 do begin
                    ...
                    if Piece(multicheck, '.', 1) = typeitem then begin
                      if Piece(multicheck, '.', 2) <> '0' then begin
    ...
    for i := aListView.Items.Count - 1 downto 0 do begin
      aListItem := aListView.Items[i];
      aGraphItem := TGraphItem(aListItem.SubItems.Objects[3]);
      typeitem := UpperCase(Pieces(aGraphItem.Values, '^', 1, 2));
      for j := 0 to aList.Count - 1 do begin
        if typeitem = aList[j] then begin
  ...
  for i := 0 to aListView.Items.Count - 1 do begin
    ...
    if ok then begin
      ...
      while not done do begin
        ...
        else if Pieces(GtslCheck[j], '^', 1, 2) = typeitem then begin
          previous := true;
          done := true;
        end else begin
          j := j + 1;
...
begin
  ...
  if not FFastTrack then
    if GraphTurboOn then
      Switch;
  if lvwItemsBottom.SelCount > FGraphSetting.MaxSelect then begin
    ...
    GtslSelPrevBottomFloat.Clear;
    for i := 0 to lvwItemsBottom.Items.Count - 1 do begin
      if lvwItemsBottom.Items[i].Selected then begin
...
begin
  for i := 0 to aListView.Items.Count - 1 do begin
    ...
    for j := 0 to aList.Count - 1 do begin
      ...
      if itemtype = typeitem then begin
        aListView.Items[i].Selected := true;
        break;
      end;
      if typenum = '63' then begin
        if Piece(itemtype, '.', 1) = Piece(typeitem, '.', 1) then begin
          aListView.Items[i].Selected := true;
          break;
        end;
      end;
...
begin
  ...
  if aListBox.Tag = BIG_NUMBER then begin           // avoids recurssion
    exit;
  end;
  if pos(LLS_FRONT, aListBox.Items[aListBox.ItemIndex]) > 0 then begin // <clear all selections>
    if aListBox.Tag = BIG_NUMBER then begin            // avoids recurssion
      exit;
    end;
  ...
  if (length(Piece(aProfile, '^', 3)) = 0) or (length(Piece(aProfile, '^', 1)) = 0) or
    (Piece(aProfile, '^', 1) = VIEW_LABS) then begin       //or <custom>
...
begin
  ...
  for j := 1 to BIG_NUMBER do begin
    ...
    if (itempart1 = '50.605') and (length(itemdrugclass) > 0) then begin
      dcnm := DCName(itempart2);
      if dcnm = itemdrugclass then begin
        Result := aName;
        break;
      end;
    end else if itempart1 = '63' then begin
      if itemnums = Piece(itemstringnums, '.', 1) then begin
        Result := aName;
        break;
      end;
    end else begin
      if itemnums = itemstringnums then begin
        Result := aName;
        break;
      end;
    end;
    if (itempart1 = '0') and (itempart2 = itemstring1) then begin
      Result := aName;
      break;
    end
...
begin
  ...
  if length(newmult) > 0 then begin
  ...
  if selectlab then begin
    if not FFastLabs then begin
      if not FPointClick then begin   // do not select test if expanding data from a point click
...
begin
  ...
  for i := 0 to aListView.Items.Count - 1 do begin
    ...
    if aItemType = checkitem then begin
      oldlisting := i;
...
begin
  ...
  if singlespec then begin
    ...
    for i := 0 to GtslScratchLab.Count - 1 do begin
    ...
    lvwItemsTop.Items.BeginUpdate;
    for i := 0 to GtslMultiSpec.Count - 1 do begin
      checktest := UpperCase(Pieces(GtslMultiSpec[i], '^', 1, 2));
      if Piece(checktest, '.', 2) = '0' then begin      // flag expanded test
        checktypeitem := Piece(checktest, '.', 1);
        for j := 0 to GtslCheck.Count - 1 do begin
          if GtslCheck[j] = checktypeitem then begin
      ...
      if (FGraphSetting.FMStartDate = FM_START_DATE) or
        DateRangeMultiItems(FGraphSetting.FMStartDate, FGraphSetting.FMStopDate, Piece(GtslMultiSpec[i], '^', 2)) then begin
...
begin
  inherited;
  EditIEN := frmNotes.ActiveEditIEN;
  if EditIEN <= 0 then begin
    MessageDlg('Images must be attached to a note.'+CRLF+
               'However, no note is currently being edited.'+CRLF+
               'Unable to save screenshot to image.' + CRLF +
               'Please start editing a note and try again.', mtError, [mbOK], 0);
...
begin
  ClearLvwItems; //kt added 6/16
  FSearchTerms.Free; //kt 6/16
  SearchDelayTimer.Free; //kt added 6/16
  SetSize;
